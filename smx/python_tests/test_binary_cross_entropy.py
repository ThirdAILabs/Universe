...
def bce_loss(dense_labels, dense_y):
    return -np.sum(
        np.log(dense_y) * dense_labels + np.log(1 - dense_y) * (1 - dense_labels)
    ) / np.prod(dense_y.shape[:-1])


def test_binary_cross_entropy_dense_single_label():
    M, K, N = 20, 10, 30
...
def test_binary_cross_entropy_sparse_single_label():
    offsets = [0, 4, 7, 12, 14]
    ...
    y4 = sigmoid_np(x4)
    expected_loss = (
        bce_loss(dense_labels=np.array([0, 0, 1, 0]), dense_y=y1)
        + bce_loss(dense_labels=np.array([0, 1, 0]), dense_y=y2)
        + bce_loss(dense_labels=np.array([0, 0, 0, 0, 1]), dense_y=y3)
        + bce_loss(dense_labels=np.array([1, 0]), dense_y=y4)
    ) / 4
...
def test_binary_cross_entropy_dense_multi_label():
    ...
    assert np.allclose(x.grad.numpy(), (l_dense - y_np) / 4)


def test_binary_cross_entropy_sparse_multi_label():
    offsets = [0, 4, 7, 12, 14]
    ...
    expected_loss = (
        bce_loss(dense_labels=np.array(m1 + m2 + m3 + m4), dense_y=full_acts) / 4
    )
    ...
    expected_grad = (np.array(m1 + m2 + m3 + m4) - full_acts) / 4
