...
def test_smx_embeddings():
    ...
    out = smx.embedding(smx.Variable(indices, requires_grad=False), embs)

    expected_out = np.stack(
        [
            (1 * embs_np[5] + 2 * embs_np[3] + 3 * embs_np[4]) / 3,
            (1 * embs_np[7] + 2 * embs_np[11] + 3 * embs_np[19] + 4 * embs_np[15]) / 4,
            (1 * embs_np[6] + 2 * embs_np[1]) / 2,
            (1 * embs_np[16] + 2 * embs_np[18] + 3 * embs_np[0] + 4 * embs_np[8]) / 4,
        ],
        axis=0,
    )
    ...
    expected_grad[5] = 1 * out_grad_np[0] / 3
    expected_grad[3] = 2 * out_grad_np[0] / 3
    expected_grad[4] = 3 * out_grad_np[0] / 3

    expected_grad[7] = 1 * out_grad_np[1] / 4
    expected_grad[11] = 2 * out_grad_np[1] / 4
    expected_grad[19] = 3 * out_grad_np[1] / 4
    expected_grad[15] = 4 * out_grad_np[1] / 4

    expected_grad[6] = 1 * out_grad_np[2] / 2
    expected_grad[1] = 2 * out_grad_np[2] / 2

    expected_grad[16] = 1 * out_grad_np[3] / 4
    expected_grad[18] = 2 * out_grad_np[3] / 4
    expected_grad[0] = 3 * out_grad_np[3] / 4
    expected_grad[8] = 4 * out_grad_np[3] / 4
