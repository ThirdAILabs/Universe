...
def parseURL(url,responseURL,skipwords,domain_type="default",restrict_to_domain=False):

    if url is None or (restrict_to_domain and get_domain(responseURL) not in url and url[0] != '/') or any(word in url for word in skipwords) or len(url)<5 or url[0]=="#":
        return None
...
def parseText(response,tags=['p'],is_scrapy_response=True):
    ...
    if is_scrapy_response:
        dc["metadata"]={'depth':response.meta['depth'],'page_no':response.meta["pages"],"url":response.request.url,}
    for tag in tags:
        paras=response.xpath(f'//{tag}')
        dc[tag]=[]
        # print(response.request.url)
        for i,para in enumerate(paras):
            data=" ".join([x.strip() for x in para.css('*::text').extract() if len(x)>2])
