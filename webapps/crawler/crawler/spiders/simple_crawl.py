...
class MySpider(scrapy.Spider):
    ...
    skipwords=["pinterest","twitter","login","facebook","youtube","ytube","shopheart"]
    # args_file=None
    custom_settings = {
    ...
    def start_requests(self):
        ...
        if self.fetch_html:
            ...
            print(f"crawling started with the parameters:\n" 
            ...
            for url in urls:
                ...
                yield scrapy.Request(url=url,meta={"depth":0,"pages":1,"folder":self.html_output_folder,"save_html":self.fetch_html,"parent":"NA"},callback=self.fetch_htmls)
    ...
    def fetch_htmls(self,response):
        ...
        if any(word in response.request.url for word in self.skipwords) or response.status!=200 or response.meta["depth"]>self.max_depth or len(self.crawled_urls)==self.max_pages :
            return
        
        self.crawled_urls.add(response.request.url)
        yield {'depth':response.meta['depth'],'page_no':response.meta['pages'],'parent':response.meta["parent"],
        ...
        for tag in tag_list:
            ...
            for i, urls in enumerate(links):
                ...
                url=parseURL(url,response.request.url,self.skipwords,restrict_to_domain=False)
                ...
                yield scrapy.Request(url=url,meta={"parent":response.request.url,"depth":response.meta["depth"]+1,"pages":j+response.meta['pages'],"folder":response.meta["folder"],"save_html":response.meta["save_html"]},callback=self.fetch_htmls)
        
        name_html=""
        if response.meta["save_html"]:
            name_html=re.sub("/","_",f"{response.request.url}_depth_{response.meta['depth']}_pageno_{response.meta['pages']}")
            if len(name_html)>200:
                name_html="random_html_"+''.join(random.choices(string.ascii_uppercase + string.digits, k=15))+".html"
                with open(f"{response.meta['folder']}/"+name_html,"wb") as f:
                    f.write(f"{response.request.url}/n".encode('utf-8')+response.body)
            else:
                with open(f"{response.meta['folder']}/"+name_html+".html","wb") as f:
        ...
        if self.parse_dynamic or self.parse_htmls:
            ...
            if self.parse_htmls:
                if name_html == "":
                    # print("here")
                    name_html=re.sub("/","_",f"{response.request.url}_depth_{response.meta['depth']}_pageno_{response.meta['pages']}").sub(".html","")
                    if len(name_html)>200:
                        name_html="random_html_"+''.join(random.choices(string.ascii_uppercase + string.digits, k=15))+".html"
