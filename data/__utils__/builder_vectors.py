...
class DenseBuilderVector(BuilderVector):
  ...
  def addSingleFeature(self, start_dim: int, value: float) -> None:
    ...
    if len(self._values > start_dim):
      raise RuntimeError(f"DenseBuilderVector: Adding feature at dimension {start_dim} but vector is already {len(self._values)} dimensions.")
    if len(self._values) < start_dim:
  ...
  def addSparseFeatures(self, indices: np.ndarray, values: np.ndarray) -> None:
    ...
    """
    raise RuntimeError("DenseBuilderVector: Attempted to add sparse features. Use SparseBuilderVector instead.")
