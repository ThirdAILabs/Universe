...
// NWPDualTokenizerConfig(std::string input_tokenizer,
...
NextWordPredictionDualTokenizer::NextWordPredictionDualTokenizer(
    ...
    ColumnMap NextWordPredictionDualTokenizer::apply(ColumnMap columns,
      ...
      std::vector<std::vector<uint32_t>> contexts(texts->numRows());
...
#pragma omp parallel for default(none) \
      ...
      for (size_t i = 0; i < texts->numRows(); i += 1) {
        auto input_text = texts->row(i);
        std::vector<uint32_t> offsets =
            _output_tokenizer.getOffsets(input_text);
      ...
      for (size_t i = 0; i < texts->numRows(); i += 1) {
        try {
          ...
          for (size_t start = 1; start < target_tokens.size(); start += 1) {
            uint32_t token_start = target_offsets[start];
...
#pragma omp critical
    ...
    std::vector<size_t> NextWordPredictionDualTokenizer::computeOffsets(
      ...
      std::vector<size_t> offsets(offsets.size() + 1);
      offsets[0] = 0;
      for (size_t i = 0; i < offsets.size(); i++) {
        offsets[i + 1] = offsets[i] + offsets[i].size() - 1;
      }
      return offsets;
    }
