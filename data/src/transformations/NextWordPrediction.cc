...
NextWordPrediction::NextWordPrediction(
    ...
    std::string target_column, std::optional<std::string> text_input_column)
...
ColumnMap NextWordPrediction::apply(ColumnMap columns, State& state) const {
  ...
  std::vector<std::string> text_contexts;
...
#pragma omp parallel for default(none)                                        \
    shared(input_tokens, token_contexts, text_contexts, targets, input_texts, \
  ...
  for (size_t i = 0; i < input_tokens->numRows(); i += 1) {
    try {
      ...
      if (_text_input_column) {
        ...
        text_row.insert(text_row.end(), input_text_row.begin(),
                        input_text_row.end());
      }

      std::vector<uint32_t> tokens_row;
      tokens_row.insert(tokens_row.end(), input_token_row.begin(),
                        input_token_row.end());
      ...
      for (size_t start = 1; start < tokens_row.size(); start += 1) {
        if (_text_input_column) {
          ...
          query_text += text_row[start];
          text_contexts[i] = query_text;
        } else {
...
#pragma omp critical
  ...
  if (_text_input_column) {
    output_columns[_context_column] =
        ValueColumn<std::string>::make(std::move(text_contexts));
  } else {
    output_columns[_context_column] = ArrayColumn<uint32_t>::make(
