...
RulePtr datePattern() {
  ...
  // mm/yyyy or m/yyyy
  rules.push_back(Pattern::make("DATE", R"(\b(([1-9]|0[1-9]|1[0-2])/\d{4})\b)",
                                0.2,
                                {
                                    {"date", 0.3},
                                    {"birthday", 0.2},
  ...
  // mm/yy or m/yy
  rules.push_back(Pattern::make("DATE", R"(\b(([1-9]|0[1-9]|1[0-2])/\d{2})\b)",
                                0.1,
                                {
                                    {"date", 0.3},
                                    {"birthday", 0.2},
...
RulePtr timePattern() {
  ...
  rules.push_back(Pattern::make(
      "TIME", R"(\b([01]?\d|2[0-3]):[0-5]\d:[0-5]\d(?:\s?[APap][Mm])?\b)", 0.6,
      {
          {"time", 0.3},
          {"at", 0.1},
          {"clock", 0.1},
...
RulePtr dateTimePattern() {
  return Pattern::make(
      /*entity=*/"DATETIME",
      /*pattern=*/
      R"(\b(?:([1-9]|0[1-9]|1[0-2])[-\/.]([1-9]|0[1-9]|[12]\d|3[0-1])[-\/.](?:\d{4}|\d{2})[ T](?:[01]?\d|2[0-3]):[0-5]\d(?:\:[0-5]\d)?(?:\s?[APap][Mm])?)\b)",
      /*pattern_score=*/0.6,
      /*context_keywords=*/
      {
          {"date", 0.3},
