...
std::optional<ValidatorSubMatch> ibanChecksum(const std::string& token) {
   ...
   *
   * 1. Move first four digits to the end.
  ...
  if (value % 97 == 1) {
    return ValidatorSubMatch(match.position(), match.length());
  }
