...
struct VariableLengthConfig {
  explicit VariableLengthConfig(
      ...
      uint32_t num_slices = 5, uint32_t strong_sample_num_words = 3,
      std::optional<uint32_t> forced_num_phrases_per_doc = std::nullopt,
      ...
      : covering_min_length(covering_min_length),
        ...
        forced_num_phrases_per_doc(forced_num_phrases_per_doc),
  ...
  std::optional<uint32_t> forced_num_phrases_per_doc;
...
class VariableLengthColdStart : public Transformation {
 ...
 private:
  ...
  std::optional<uint32_t> _forced_num_phrases_per_doc;
