...
#pragma omp parallel for default(none)                                    \
    shared(texts, sample_offsets, interval_from_end, interval_from_start, \
           prompts, prompt_inputs, current_context_lengths, targets, chunk_size, error)
...
if(_context_length_column){
    output_columns[*_context_length_column] =
      ValueColumn<uint32_t>::make(std::move(current_context_lengths), chunk_size);
}
