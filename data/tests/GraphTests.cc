...
TransformationPtr getBuilder(bool serialize = false) {
  ...
  if (serialize) {
    ...
    return Transformation::deserialize(
        std::dynamic_pointer_cast<Transformation>(builder)->serialize());
  }
...
TEST(GraphTests, NeighborIdsSerialization) {
  ...
  // the cereal "serialize" method. This can be removed once cereal is
  // officially depreciated.
  auto new_transform = Transformation::deserialize(
      dynamic_cast<Transformation*>(&neighbor_ids)->serialize());
...
TEST(GraphTests, NeighborFeaturesSerialization) {
  ...
  // officially depreciated.
  auto new_transform = Transformation::deserialize(
      dynamic_cast<Transformation*>(&neighbor_features)->serialize());
