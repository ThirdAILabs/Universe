...
def test_all_reduce_circular_communication():
    ...
    for i in range(num_workers):
        circular_communicating_workers[i].w_gradients = [
            (i + 1) * np.ones(weight_matrix_shape)
        ]
        ...
        bias_all_reduced += circular_communicating_workers[i].b_gradients[0]
    ...
    for node in range(num_workers - 1):
        if node == num_workers - 2:
            for worker_id in range(num_workers):
                ...
                w_gradients, b_gradients = circular_communicating_workers[
                    worker_id
                ].friend.receive_array_partitions(update_id)
                circular_communicating_workers[
                    worker_id
                ].friend_weight_gradient_list = w_gradients
                circular_communicating_workers[
                    worker_id
                ].friend_bias_gradient_list = b_gradients
                circular_communicating_workers[worker_id].update_partitions(
    ...
    for i in range(num_workers):
        CHECK_PARAMS = (
            CHECK_PARAMS
            and (
                weight_all_reduced == circular_communicating_workers[i].w_gradients
            ).all()
        )
        CHECK_PARAMS = (
            CHECK_PARAMS
            and (
                bias_all_reduced == circular_communicating_workers[i].b_gradients
            ).all()
        )
