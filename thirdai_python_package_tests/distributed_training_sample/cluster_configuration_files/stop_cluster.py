...
def init_cluster_config():
    ...
    config = {
        ...
        "provider": {"type": "local", "head_ip": None, "worker_ips": []},
        "auth": {"ssh_user": ""},
        "file_mounts": {},
        "initialization_commands": [],
        "setup_commands": ["pip install 'ray[default]'"],
        "head_setup_commands": [],
        "worker_setup_commands": [],
        "head_start_ray_commands": ["ray stop", "ray start --head --port=6379"],
...
def start_cluster(nodes_ips) -> None:
    ...
    subprocess.run(
        ["sh", "destroy_cluster.sh", " ".join(nodes_ips[1 : len(nodes_ips)])]
    )
