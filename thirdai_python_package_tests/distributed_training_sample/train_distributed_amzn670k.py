...
head = DistributedBolt(2, config_filename, pregenerate=True, logfile=f"logfile_experiments_{config_filename.split('.')[1][1:]}.log")

if sys.argv[2]=="None":
    head.train(
        circular=False, compression=None,
    )
else:
    head.train(
        circular=False, compression=sys.argv[2], compression_density=0.05, scheduler=False
    )
acc, _ = head.predict()
