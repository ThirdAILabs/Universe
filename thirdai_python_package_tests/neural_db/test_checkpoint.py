...
def on_progress(percent_training_completed):
    # This is used to stop the training at a random point.
    if percent_training_completed > random.random():
...
def interrupted_training(number_models: int):
...
def test_interrupted_training_mach():
    interrupted_training(number_models=1)


def test_interrupted_training_mach_mixture():
    interrupted_training(number_models=3)
