...
bool SymSpell::LoadBigramDictionary(string corpus, uint32_t termIndex, uint32_t countIndex, xchar separatorChars)
{
...
/// <param name="separatorChars">Separator characters between term(s) and count.</param>
/// <returns>True if file loaded, or false if file not found.</returns>
bool SymSpell::LoadBigramDictionary(xifstream& corpusStream, uint32_t termIndex, uint32_t countIndex, xchar separatorChars)
{
	xstring line;
...
/// <param name="separatorChars">Separator characters between term(s) and count.</param>
/// <returns>True if file loaded, or false if file not found.</returns>
bool SymSpell::LoadDictionary(string corpus, uint32_t termIndex, uint32_t countIndex, xchar separatorChars)
{
...
bool SymSpell::LoadDictionary(xifstream& corpusStream, uint32_t termIndex, uint32_t countIndex, xchar separatorChars)
{
...
/// <returns>A vector of SuggestItem object representing suggested correct spellings for the input word, 
/// sorted by edit distance, and secondarily by count frequency.</returns>
vector<SuggestItem> SymSpell::Lookup(xstring input, Verbosity verbosity, uint32_t maxEditDistance)
{
...
/// <returns>A vector of SuggestItem object representing suggested correct spellings for the input word, 
/// sorted by edit distance, and secondarily by count frequency.</returns>
vector<SuggestItem> SymSpell::Lookup(xstring input, Verbosity verbosity, uint32_t maxEditDistance, bool includeUnknown)
{
	...
	if ((uint32_t)inputLen > this->maxDictionaryWordLength + maxEditDistance) skip = 1;
		...
		while (candidatePointer < candidates.size())
		{
			...
			if (deletes != nullptr and deletes->count(GetstringHash(candidate)))
			{
				...
				for (size_t i = 0; i < dictSuggestions.size(); i++)
				{
					...
					if (suggPrefixLen > (int)inputPrefixLen && (suggPrefixLen - candidateLen) > maxEditDistance2) continue;
					...
					else if (suggestionLen == 1)
					{
						...
						if (((prefixLength  == (uint32_t)candidateLen + maxEditDistance)
							&& (((min_len = min(inputLen, suggestionLen) - (int)prefixLength) > 1)
			...
			if (((uint32_t)lengthDiff < maxEditDistance) && ((uint32_t)candidateLen <= prefixLength))
			{
				//save some time
	...
	bool SymSpell::DeleteInSuggestionPrefix(xstring deleteSugg, uint32_t deleteLen, xstring suggestion, uint32_t suggestionLen)
	{
	...
	HashSet<xstring>* SymSpell::Edits(xstring word, uint32_t editDistance, HashSet<xstring>* deleteWords)
	{
		editDistance++;
		if (word.size() > 1)
		{
	...
	/// <param name="maxEditDistance">The maximum edit distance between input and suggested words.</param>																											   
	/// <returns>A vector of SuggestItem object representing suggested correct spellings for the input string.</returns> 
	vector<SuggestItem> SymSpell::LookupCompound(xstring input, uint32_t editDistanceMax)
	{
	...
	/// the Edit distance sum between input string and corrected string, 
	/// the Sum of word occurence probabilities in log scale (a measure of how common and probable the corrected segmentation is).</returns> 
	Info SymSpell::WordSegmentation(xstring input, uint32_t maxEditDistance, uint32_t maxSegmentationWordLength)
	{
		...
		for (uint32_t j = 0; j < input.size(); j++)
		{
			...
			circularIndex++; if (circularIndex == (int)arraySize) circularIndex = 0;
		}
