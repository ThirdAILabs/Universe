 ...
 * bit range.
 */
constexpr uint32_t randDoubleHash(uint32_t n1, uint32_t n2,
 ...
 */
constexpr uint32_t fastDoubleHash(uint32_t n1, uint32_t n2,
                                  uint32_t bit_range) {
 ...
 * so don't do that.
 */
inline void compactHashBits(const uint32_t* hashes, uint32_t* output_hashes,
                            uint32_t length_output,
                            uint32_t hashes_per_output_value) {
  for (uint32_t i = 0; i < length_output; i++) {
 ...
 * filled.
 */
inline void densifyHashesPowerOf2(uint32_t* hashes, uint32_t log_num_hashes,
 ...
 */
inline uint32_t combineHashes(uint32_t lhs, uint32_t rhs) {
 ...
 */
inline uint32_t simpleIntegerHash(uint32_t val) {
