...
class HashModP(tf.Module):
    ...
    def __init__(self, num_hashes, prime=2**31 - 1, seed=None):
        ...
        self._a = tf.Variable(initializer_a(shape=[self._num_hashes], dtype=tf.int64))
        self._b = tf.Variable(initializer_b(shape=[self._num_hashes], dtype=tf.int64))
...
class StringMinHash(tf.Module):
    ...
    def __init__(self, num_hashes, num_buckets=2**32 - 1, salt_len=4, seed=None):
        """Hash function constructor.
        ...
        if num_hashes < 1:
            raise ValueError("num_hashes must be >= 1 but is %s" % num_hashes)
        if num_buckets < 2:
            raise ValueError("num_buckets must be >= 2 " "but is %s" % num_buckets)
        if salt_len < 1:
        ...
        if seed:
            ...
            random.seed(seed)
        self._salts = [
            "".join(random.choice(string.printable) for _ in range(salt_len))
    ...
    def hash(self, x):
        ...
        hashed_strings = [
            tf.strings.to_hash_bucket_fast(s, self._num_buckets) for s in salted_strings
        ]
...
class PStableHash(tf.Module):
    ...
    def __init__(self, dimension, num_hashes, scale=1.0, p=2.0, seed=None):
        """Hash function constructor.
    ...
    def _init_projections(self):
        ...
        if self._p == 2.0:  # Special case: Gaussian-distributed projection
            ...
            return tf.Variable(initializer(shape=[self._dim, self._num_hashes]))
            ...
            x0 = tf.Variable(initializer_0(shape=[self._dim, self._num_hashes]))
            x1 = tf.Variable(initializer_1(shape=[self._dim, self._num_hashes]))
    ...
    def hash(self, x):
        ...
        return tf.cast(rounded_projs, tf.int32)


class SRPHash(tf.Module):
    """signed random projection hash for the angular distance metric."""

    def __init__(self, dimension, num_hashes, num_bits=1, seed=None):
        """Hash function constructor.
