...
class TriGramVectorizer : public StringVectorizer {
 public:
  TriGramVectorizer() {
    _dim = 50653; // 37 ^ 3; 26 letters in the alphabet, 10 numbers, and space. 3 characters per token.
  };
  ...
  virtual void vectorize(const std::string& str,
    ...
    for (size_t i = 0; i < len - 2; i++) {
      // Using MurmurHash instead of a rolling hash because 3 characters take up less than 32 bits, 
      // so this is only one iteration in MurmurHash, and it has very good distribution.
