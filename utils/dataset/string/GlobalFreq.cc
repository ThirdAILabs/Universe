...
GlobalFreq::GlobalFreq(std::vector<std::string>& files,
                       StringLoader* string_loader, u_int32_t murmur_seed) {
  ...
  for (auto file : files) {
    std::string buffer;
    // TODO: (henry) Make sure we really need this line because StringDataset
    // already calls it
...
int GlobalFreq::idf_size() { return _idfMap.size(); }
