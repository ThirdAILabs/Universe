...
class StringDataset : public Dataset {
 public:
  // n-gram constructor
  StringDataset(std::string filename, uint64_t target_batch_size,
                uint64_t target_batch_num_per_load,
                STRING_TOKEN_TYPE token_type, STRING_VECTOR_TYPE vector_type,
                uint32_t n)
  ...
  virtual void loadNextBatchSet() {
    ...
    while (batch_count < _target_batch_num_per_load) {
      bool loaded =
          _loader.loadNextString(strings_to_be_vectorized[batch_count]);
      if (!loaded) {
        break;
      }
      batch_count++;
...
#pragma omp parallel for
    for (uint64_t i = 0; i < batch_count; i++) {
      _tokenizer.getTokenIds(strings_to_be_vectorized[i], _indices[i],
                          _values[i]);
    }
  }
