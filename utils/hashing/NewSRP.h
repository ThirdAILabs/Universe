...
class FastSRP {
 private:
  uint32_t _hashes_per_table, _num_tables, num_hashes, _log_num_hashes, _dim, _binsize, _permute;
  uint32_t* _bin_map;
  uint32_t* _positions;
 ...
 public:
  FastSRP(uint32_t input_dim, uint32_t _hashes_per_table, uint32_t _num_tables, uint32_t range_pow);
