...
/// <param name="initialCapacity">The expected number of words in
/// dictionary.</param> <param name="maxDictionaryEditDistance">Maximum edit
/// distance for doing lookups.</param> <param name="prefixLength">The length of
/// word prefixes used for spell checking..</param> <param
/// name="countThreshold">The minimum frequency count for dictionary words to be
/// considered correct spellings.</param> <param name="compactLevel">Degree of
/// favoring lower memory use over speed (0=fastest,most memory,
/// 16=slowest,least memory).</param>
SymSpell::SymSpell(int initialCapacity, int maxDictionaryEditDistance,
                   int prefixLength, int countThreshold,
...
/// <remarks>For every word there are deletes with an edit distance
/// of 1..maxEditDistance created and added to the dictionary. Every delete
/// entry has a suggestions list, which points to the original term(s) it was
/// created from. The dictionary may be dynamically updated (word frequency and
/// new words) at any time by calling CreateDictionaryEntry</remarks> <param
/// name="key">The word to add to dictionary.</param> <param name="count">The
/// frequency count for word.</param> <param name="staging">Optional staging
/// object to speed up adding many entries by staging them to a temporary
/// structure.</param> <returns>True if the word was added as a new correctly
/// spelled word, or false if the word is added as a below threshold word, or
/// updates an existing correctly spelled word.</returns>
bool SymSpell::CreateDictionaryEntry(xstring key, int64_t count,
                                     SuggestionStage* staging) {
  if (count <= 0) {
    if (this->countThreshold > 0)
      return false;  // no point doing anything if count is zero, as it can't
                     // change anything
    count = 0;
  }
  int countPrevious = -1;
  // look first in below threshold words, update count, and allow promotion to
  // correct spelling word if count reaches threshold threshold must be >1 for
  // there to be the possibility of low threshold words
  auto belowThresholdWordsFinded = belowThresholdWords.find(key);
  auto wordsFinded = words.find(key);
  if (countThreshold > 1 &&
      belowThresholdWordsFinded != belowThresholdWords.end()) {
    ...
    // has reached threshold - remove from below threshold collection (it will
    // be added to correct words below)
  ...
  // corpus, even if the same term existed before in the dictionary as an edit
  // from another word
  if (key.size() > this->maxDictionaryWordLength)
    this->maxDictionaryWordLength = key.size();
  ...
  if (staging != NULL) {
    ...
    for (auto it = edits.begin(); it != edits.end(); ++it) {
      ...
      // if (deletes.TryGetValue(deleteHash, out string[] suggestions))
      {
        ...
        for (size_t id = 0; id < suggestions.size(); id++) {
...
/// <summary>Load multiple dictionary entries from a file of word/frequency
/// count pairs</summary> <remarks>Merges with any dictionary data already
/// loaded.</remarks> <param name="corpus">The path+filename of the
/// file.</param> <param name="termIndex">The column position of the
/// word.</param> <param name="countIndex">The column position of the frequency
/// count.</param> <param name="separatorChars">Separator characters between
/// term(s) and count.</param> <returns>True if file loaded, or false if file
/// not found.</returns>
bool SymSpell::LoadBigramDictionary(string corpus, uint32_t termIndex,
  ...
  return LoadBigramDictionary(corpusStream, termIndex, countIndex,
                              separatorChars);
}
...
/// term(s) and count.</param> <returns>True if file loaded, or false if file
/// not found.</returns>
bool SymSpell::LoadBigramDictionary(xifstream& corpusStream, uint32_t termIndex,
                                    uint32_t countIndex, xchar separatorChars) {
  xstring line;
  uint32_t linePartsLength = (separatorChars == DEFAULT_SEPARATOR_CHAR) ? 3 : 2;
  // process a single line at a time only for memory efficiency
  while (getline(corpusStream, line)) {
    vector<xstring> lineParts;
    xstringstream ss(line);
    xstring token;
    while (getline(ss, token, separatorChars)) lineParts.push_back(token);
    xstring key;
    int64_t count = 0;
    if (lineParts.size() >= linePartsLength) {
      // if default (whitespace) is defined as separator take 2 term parts,
      // otherwise take only one
      key = (separatorChars == DEFAULT_SEPARATOR_CHAR)
                ? lineParts[termIndex] + XL(" ") + lineParts[termIndex + 1]
                : lineParts[termIndex];
      try {
...
/// term(s) and count.</param> <returns>True if file loaded, or false if file
/// not found.</returns>
bool SymSpell::LoadDictionary(string corpus, uint32_t termIndex,
                              uint32_t countIndex, xchar separatorChars) {
...
/// <summary>Load multiple dictionary entries from a stream of word/frequency
/// count pairs</summary> <remarks>Merges with any dictionary data already
/// loaded.</remarks> <param name="corpusStream">The stream containing the
/// word/frequency count pairs.</param> <param name="termIndex">The column
/// position of the word.</param> <param name="countIndex">The column position
/// of the frequency count.</param> <param name="separatorChars">Separator
/// characters between term(s) and count.</param> <returns>True if stream
/// loads.</returns>
bool SymSpell::LoadDictionary(xifstream& corpusStream, uint32_t termIndex,
                              uint32_t countIndex, xchar separatorChars) {
...
/// <summary>Load multiple dictionary words from a file containing plain
/// text.</summary> <remarks>Merges with any dictionary data already
/// loaded.</remarks> <param name="corpus">The path+filename of the
/// file.</param> <returns>True if file loaded, or false if file not
/// found.</returns>
...
/// <summary>Load multiple dictionary words from a stream containing plain
/// text.</summary> <remarks>Merges with any dictionary data already
/// loaded.</remarks> <param name="corpusStream">The stream containing the plain
/// text.</param> <returns>True if stream loads.</returns>
...
/// <remarks>This can be used to reduce memory consumption after populating the
/// dictionary from a corpus using CreateDictionary.</remarks>
void SymSpell::PurgeBelowThresholdWords() { belowThresholdWords.clear(); }

/// <summary>Commit staged dictionary additions.</summary>
/// <remarks>Used when you write your own process to load multiple words into
/// the dictionary, and as part of that process, you first created a
/// SuggestionsStage object, and passed that to CreateDictionaryEntry
/// calls.</remarks> <param name="staging">The SuggestionStage object storing
/// the staged data.</param>
...
/// edit distance specified during construction of the SymSpell
/// dictionary.</summary> <param name="input">The word being spell
/// checked.</param> <param name="verbosity">The value controlling the
/// quantity/closeness of the retuned suggestions.</param> <returns>A vector of
/// SuggestItem object representing suggested correct spellings for the input
/// word, sorted by edit distance, and secondarily by count frequency.</returns>
...
/// dictionary.</summary> <param name="input">The word being spell
/// checked.</param> <param name="verbosity">The value controlling the
/// quantity/closeness of the retuned suggestions.</param> <param
/// name="maxEditDistance">The maximum edit distance between input and suggested
/// words.</param> <returns>A vector of SuggestItem object representing
/// suggested correct spellings for the input word, sorted by edit distance, and
/// secondarily by count frequency.</returns>
vector<SuggestItem> SymSpell::Lookup(xstring input, Verbosity verbosity,
...
/// <param name="verbosity">The value controlling the quantity/closeness of the
/// retuned suggestions.</param> <param name="maxEditDistance">The maximum edit
/// distance between input and suggested words.</param> <param
/// name="includeUnknown">Include input word in suggestions, if no words within
/// edit distance found.</param> <returns>A vector of SuggestItem object
/// representing suggested correct spellings for the input word, sorted by edit
/// distance, and secondarily by count frequency.</returns>
vector<SuggestItem> SymSpell::Lookup(xstring input, Verbosity verbosity,
                                     uint32_t maxEditDistance,
                                     bool includeUnknown) {
  // verbosity=Top: the suggestion with the highest term frequency of the
  ...
  if ((uint32_t)inputLen > this->maxDictionaryWordLength + maxEditDistance)
    skip = 1;
  ...
  // early termination, if we only want to check if word in dictionary or get
  ...
  if (!skip) {
    ...
    uint32_t candidatePointer = 0;
    vector<xstring> singleSuggestion = {XL("")};
    vector<xstring> candidates;

    // add original prefix
    uint32_t inputPrefixLen = inputLen;
    ...
    while (candidatePointer < candidates.size()) {
      ...
      // if candidate distance is already higher than suggestion distance, than
      // there are no better suggestions to be expected
      if (lengthDiff > maxEditDistance2) {
        // skip to next candidate if Verbosity.All, look no further if
        // Verbosity.Top or Closest (candidates are ordered by delete distance,
        // so none are closer than current)
      ...
      if (deletes != nullptr and deletes->count(GetstringHash(candidate))) {
        ...
        // iterate through suggestions (to other correct dictionary items) of
        // delete item and add them to suggestion list
        for (size_t i = 0; i < dictSuggestions.size(); i++) {
          ...
          if ((abs(suggestionLen - inputLen) >
               maxEditDistance2)  // input and sugg lengths diff >
                                  // allowed/current best distance
              || (suggestionLen <
                  candidateLen)  // sugg must be for a different delete string,
                                 // in same bin only because of hash collision
              ||
              (suggestionLen == candidateLen &&
               suggestion != candidate))  // if sugg len = delete len, then it
                                          // either equals delete or is in same
                                          // bin only because of hash collision
            continue;
          auto suggPrefixLen = min(suggestionLen, (int)prefixLength);
          if (suggPrefixLen > (int)inputPrefixLen &&
          ...
          int min_len = 0;
          if (candidateLen == 0) {
            // suggestions which have no common chars with input
            // (inputLen<=maxEditDistance && suggestionLen<=maxEditDistance)
          ...
          // verbosity<All (note: maxEditDistance2 will always equal
          // maxEditDistance when Verbosity.All)
          if (distance <= maxEditDistance2) {
            ...
            if (suggestions.size() > 0) {
              switch (verbosity) {
                case Closest: {
                  // we will calculate DamLev distance only to the smallest
      ...
      // candidates list this is a recursive process until the maximum edit
      // distance has been reached
      if (((uint32_t)lengthDiff < maxEditDistance) &&
          ((uint32_t)candidateLen <= prefixLength)) {
        // save some time
        // do not create edits with edit distance smaller than suggestions
        // already found
...
bool SymSpell::DeleteInSuggestionPrefix(xstring deleteSugg, uint32_t deleteLen,
                                        xstring suggestion,
                                        uint32_t suggestionLen) {
...
vector<xstring> SymSpell::ParseWords(xstring text) {
  ...
  // characters and digits) plus "_" \d Digits Compatible with non-latin
  // characters, does not split words at apostrophes
  xregex r(XL("['’\\w\\-\\[_\\]]+"));
  xsmatch m;
  vector<xstring> matches;
  // for benchmarking only: with CreateDictionary("big.txt","") and the text
  // corpus from http://norvig.com/big.txt  the Regex below provides the exact
  // same number of dictionary items as Norvigs regex "[a-z]+" (which splits
  // words at apostrophes & incompatible with non-latin characters)
...
HashSet<xstring>* SymSpell::Edits(xstring word, uint32_t editDistance,
...
// 2. mistakenly omitted space between two correct words led to one incorrect
// combined term
// 3. multiple independent input terms with/without spelling errors

/// <summary>Find suggested spellings for a multi-word input string (supports
/// word splitting/merging).</summary> <param name="input">The string being
/// spell checked.</param> <returns>A vector of SuggestItem object representing
/// suggested correct spellings for the input string.</returns>
...
/// <summary>Find suggested spellings for a multi-word input string (supports
/// word splitting/merging).</summary> <param name="input">The string being
/// spell checked.</param> <param name="maxEditDistance">The maximum edit
/// distance between input and suggested words.</param> <returns>A vector of
/// SuggestItem object representing suggested correct spellings for the input
/// string.</returns>
vector<SuggestItem> SymSpell::LookupCompound(xstring input,
                                             uint32_t editDistanceMax) {
  ...
  for (size_t i = 0; i < termList1.size(); i++) {
    ...
    if ((i > 0) && !lastCombi) {
      ...
      if (suggestionsCombi.size() > 0) {
        ...
        if (suggestions.size() > 0) {
          ...
          best2.count =
              (long)((double)10 /
                     pow((double)10, (double)best2.term.size()));  // 0;
        }

        // distance1=edit distance between 2 split terms und their best
        // corrections : als comparative value for the combination
    ...
    // alway split terms without suggestion / never split terms with suggestion
    // ed=0 / never split single char terms
    if ((suggestions.size() > 0) &&
        ((suggestions[0].distance == 0) || (termList1[i].size() == 1))) {
      ...
      if (termList1[i].size() > 1) {
        for (uint32_t j = 1; j < termList1[i].size(); j++) {
          xstring part1 = termList1[i].substr(0, j);
          xstring part2 = termList1[i].substr(j);
          SuggestItem suggestionSplit = SuggestItem();
          vector<SuggestItem> suggestions1 =
              Lookup(part1, Top, editDistanceMax);
          if (suggestions1.size() > 0) {
            vector<SuggestItem> suggestions2 =
                Lookup(part2, Top, editDistanceMax);
            if (suggestions2.size() > 0) {
              // select best suggestion for split pair
              suggestionSplit.term =
                  suggestions1[0].term + XL(" ") + suggestions2[0].term;

              int distance2 = distanceComparer.Compare(
              ...
              if (bigrams.count(suggestionSplit.term)) {
                ...
                // increase count, if split.corrections are part of or identical
                // to input single term correction exists
                if (suggestions.size() > 0) {
                  // alternatively remove the single term from suggestionsSplit,
                  // but then other splittings could win
                  if ((suggestions1[0].term + suggestions2[0].term ==
                       termList1[i])) {
                    // make count bigger than count of single term correction
                    suggestionSplit.count =
                        max(suggestionSplit.count, suggestions[0].count + 2);
                  } else if ((suggestions1[0].term == suggestions[0].term) ||
                             (suggestions2[0].term == suggestions[0].term)) {
                    // make count bigger than count of single term correction
                    suggestionSplit.count =
                        max(suggestionSplit.count, suggestions[0].count + 1);
                  }
                }
                // no single term correction exists
                else if ((suggestions1[0].term + suggestions2[0].term ==
                          termList1[i])) {
                  suggestionSplit.count = max(
                      suggestionSplit.count,
                      max(suggestions1[0].count, suggestions2[0].count) + 2);
                }
        ...
        if (suggestionSplitBest.count) {
          ...
          // estimated word occurrence probability P=10 / (N * 10^word length l)
          si.count =
              (long)((double)10 / pow((double)10, (double)si.term.size()));
  ...
  suggestion.term = s;
  suggestion.distance =
      distanceComparer.Compare(input, suggestion.term, MAXINT);
...
/// <summary>Find suggested spellings for a multi-word input string (supports
/// word splitting/merging).</summary> <param name="input">The string being
/// spell checked.</param> <returns>The word segmented string, the word
/// segmented and spelling corrected string, the Edit distance sum between input
/// string and corrected string, the Sum of word occurence probabilities in log
/// scale (a measure of how common and probable the corrected segmentation
/// is).</returns>
Info SymSpell::WordSegmentation(xstring input) {
  return WordSegmentation(input, this->MaxDictionaryEditDistance(),
                          this->maxDictionaryWordLength);
}

/// <summary>Find suggested spellings for a multi-word input string (supports
/// word splitting/merging).</summary> <param name="input">The string being
/// spell checked.</param> <param name="maxEditDistance">The maximum edit
/// distance between input and corrected words (0=no correction/segmentation
/// only).</param> <returns>The word segmented string, the word segmented and
/// spelling corrected string, the Edit distance sum between input string and
/// corrected string, the Sum of word occurence probabilities in log scale (a
/// measure of how common and probable the corrected segmentation is).</returns>
Info SymSpell::WordSegmentation(xstring input, uint32_t maxEditDistance) {
  return WordSegmentation(input, maxEditDistance,
                          this->maxDictionaryWordLength);
}

/// <summary>Find suggested spellings for a multi-word input string (supports
/// word splitting/merging).</summary> <param name="input">The string being
/// spell checked.</param> <param name="maxSegmentationWordLength">The maximum
/// word length that should be considered.</param> <param
/// name="maxEditDistance">The maximum edit distance between input and corrected
/// words (0=no correction/segmentation only).</param> <returns>The word
/// segmented string, the word segmented and spelling corrected string, the Edit
/// distance sum between input string and corrected string, the Sum of word
/// occurence probabilities in log scale (a measure of how common and probable
/// the corrected segmentation is).</returns>
Info SymSpell::WordSegmentation(xstring input, uint32_t maxEditDistance,
                                uint32_t maxSegmentationWordLength) {
  uint32_t arraySize = min(maxSegmentationWordLength, (uint32_t)input.size());
  vector<Info> compositions = vector<Info>(arraySize);
  int circularIndex = -1;

  // outer loop (column): all possible part start positions
  for (uint32_t j = 0; j < input.size(); j++) {
    // inner loop (row): all possible part lengths (from start position): part
    // can't be bigger than longest word in dictionary (other than long unknown
    // word)
    uint32_t imax = min((uint32_t)input.size() - j, maxSegmentationWordLength);
    for (uint32_t i = 1; i <= imax; i++) {
      ...
      if (results.size() > 0) {
        ...
        // therefore the resulting probability of the word combination is the
        ...
        // otherwise long input text would win as long unknown word (with
        // ed=edmax+1 ), although there there should many spaces inserted
        topEd += part.size();
      ...
      if (j == 0) {
        compositions[destinationIndex].set(part, topResult, topEd,
                                           topProbabilityLog);
      } else if ((i == maxSegmentationWordLength)
                 // replace values if better probabilityLogSum, if same edit
                 // distance OR one space difference
                 || (((compositions[circularIndex].getDistance() + topEd ==
                       compositions[destinationIndex].getDistance()) ||
                      (compositions[circularIndex].getDistance() +
                           separatorLength + topEd ==
                       compositions[destinationIndex].getDistance())) &&
                     (compositions[destinationIndex].getProbability() <
                      compositions[circularIndex].getProbability() +
                          topProbabilityLog))
                 ...
                 || (compositions[circularIndex].getDistance() +
                     ...
                     compositions[destinationIndex].getDistance())) {
        xstring seg =
            compositions[circularIndex].getSegmented() + XL(" ") + part;
        xstring correct =
            compositions[circularIndex].getCorrected() + XL(" ") + topResult;
        int d =
    ...
    circularIndex++;
    if (circularIndex == (int)arraySize) circularIndex = 0;
  }
  return compositions[circularIndex];
}

// CEREAL_REGISTER_TYPE(SymSpell)