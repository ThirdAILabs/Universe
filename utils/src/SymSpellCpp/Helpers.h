...
class ChunkArray {
 private:
  const int ChunkSize = 4096;  // this must be a power of 2, otherwise can't
                               // optimize Row and Col functions
  const int DivShift = 12;  // number of bits to shift right to do division by
                            // ChunkSize (the bit position of ChunkSize)
  int Row(unsigned int index) {
    return index >> DivShift;
  }  // same as index / ChunkSize
  int Col(unsigned int index) {
 ...
 public:
  ...
  T& At(unsigned int index) { return Values[Row(index)][Col(index)]; }
...
class Helpers {
 public:
  /// <summary>Determines the proper return value of an edit distance function
  /// when one or both strings are null.</summary>
  ...
  /// <summary>Determines the proper return value of an similarity function when
  /// one or both strings are null.</summary>
  ...
  /// common prefix and suffix substrings are excluded.</summary>
  /// <remarks>Expects string1.size() to be less than or equal to
  /// string2.size()</remarks>
  static void PrefixSuffixPrep(xstring string1, xstring string2, int& len1,
                               int& len2, int& start) {
    len2 = string2.size();
    len1 =
        string1.size();  // this is also the minimum length of the two strings
  ...
  /// <param name="length">The length of the longer of the two strings the edit
  /// distance is from.</param> <param name="distance">The edit distance between
  /// two strings.</param> <returns>A similarity value from 0 to 1.0 (1 -
  ...
  /// <param name="length">The length of the longer of the two strings the edit
  /// distance is from.</param> <param name="similarity">The similarity measure
  /// between two strings.</param> <returns>An edit distance from 0 to length
  /// (length * (1 - similarity)).</returns>
...
class IDistance {
 public:
  ...
  /// <returns>0 if the strings are equivalent, otherwise a positive number
  ...
  /// <param name="string1">One of the strings to compare.</param>
  /// <param name="string2">The other string to compare.</param>
  /// <param name="maxDistance">The maximum distance that is of
  /// interest.</param> <returns>-1 if the distance is greater than the
  /// maxDistance, 0 if the strings are equivalent, otherwise a positive number
  /// whose magnitude increases as difference between the strings
  /// increases.</returns>
  virtual double Distance(xstring string1, xstring string2,
                          double maxDistance) = 0;
};

/// <summary>Types implementing the ISimilarity interface provide methods
/// for computing a normalized value of similarity between two
/// strings.</summary>
class ISimilarity {
 public:
  ...
  /// <param name="string1">One of the strings to compare.</param>
  /// <param name="string2">The other string to compare.</param>
  /// <returns>The degree of similarity 0 to 1.0, where 0 represents a lack of
  /// any notable similarity, and 1 represents equivalent strings.</returns>
  virtual double Similarity(xstring string1, xstring string2) = 0;

  /// <summary>Return a measure of the similarity between two strings.</summary>
  /// <param name="string1">One of the strings to compare.</param>
  /// <param name="string2">The other string to compare.</param>
  /// <param name="minSimilarity">The minimum similarity that is of
  /// interest.</param> <returns>The degree of similarity 0 to 1.0, where -1
  /// represents a similarity lower than minSimilarity, otherwise, a number
  /// between 0 and 1.0 where 0 represents a lack of any notable similarity, and
  /// 1 represents equivalent strings.</returns>
  virtual double Similarity(xstring string1, xstring string2,
                            double minSimilarity) = 0;
};

/// <summary>
/// Class providing optimized methods for computing Damerau-Levenshtein Optimal
...
/// The Damerau-Levenshtein algorithm is basically the Levenshtein algorithm
/// with a modification that considers transposition of two adjacent characters
/// as a single edit. The optimized algorithm was described in detail in my post
/// at http://blog.softwx.net/2015/01/optimizing-damerau-levenshtein_15.html
/// Also see http://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance
/// Note that this implementation of Damerau-Levenshtein is the simpler and
/// faster optimal string alignment (aka restricted edit) distance that difers
/// slightly from the classic algorithm by imposing the restriction that no
/// substring is edited more than once. So, for example, "CA" to "ABC" has an
/// edit distance of 2 by a complete application of Damerau-Levenshtein, but has
/// a distance of 3 by the method implemented here, that uses the optimal string
/// alignment algorithm. This means that this algorithm is not a true metric
/// since it does not uphold the triangle inequality. In real use though, this
/// OSA version may be desired. Besides being faster, it does not give the lower
/// distance score for transpositions that occur across long distances. Actual
/// human error transpositions are most likely for adjacent characters. For
/// example, the classic Damerau algorithm gives a distance of 1 for these two
/// strings: "sated" and "dates" (it counts the 's' and 'd' as a single
/// transposition. The optimal string alignment version of Damerau in this class
/// gives a distance of 2 for these two strings (2 substitutions), as it only
/// counts transpositions for adjacent characters. The methods in this class are
/// not threadsafe. Use the static versions in the Distance class if that is
/// required.</remarks>
class DamerauOSA : public IDistance {
 ...
 public:
  ...
  /// <remarks>By specifying the max expected string length, better memory
  /// efficiency can be achieved.</remarks> <param
  /// name="expectedMaxstringLength">The expected maximum length of strings that
  /// will be passed to the edit distance functions.</param>
  DamerauOSA(int expectedMaxstringLength) {
    if (expectedMaxstringLength <= 0)
      throw "expectedMaxstringLength must be larger than 0";
  ...
  double Distance(xstring string1, xstring string2) {
    ...
    if (len2 > static_cast<int>(this->baseChar1Costs.size())) {
      this->baseChar1Costs = vector<int>(len2, 0);
      this->basePrevChar1Costs = vector<int>(len2, 0);
    }
    return Distance(string1, string2, len1, len2, start, this->baseChar1Costs,
  ...
  double Distance(xstring string1, xstring string2, double maxDistance) {
    ...
    if (static_cast<int>(string2.size()) - static_cast<int>(string1.size()) >
        iMaxDistance)
      return -1;
    ...
    if (iMaxDistance < len2) {
      return Distance(string1, string2, len1, len2, start, iMaxDistance,
                      this->baseChar1Costs, this->basePrevChar1Costs);
    }
    return Distance(string1, string2, len1, len2, start, this->baseChar1Costs,
                    this->basePrevChar1Costs);
  }
  ...
  /// between two strings (1 - (damerau distance / len of longer
  /// string)).</summary> <param name="string1">One of the strings to
  /// compare.</param> <param name="string2">The other string to
  /// compare.</param> <returns>The degree of similarity 0 to 1.0, where 0
  ...
  double Similarity(xstring string1, xstring string2) {
    ...
    return Helpers::ToSimilarity(
        Distance(string1, string2, len1, len2, start, this->baseChar1Costs,
                 this->basePrevChar1Costs),
        string2.size());
  }
  ...
  /// string)).</summary> <param name="string1">One of the strings to
  /// compare.</param> <param name="string2">The other string to
  /// compare.</param> <param name="minSimilarity">The minimum similarity that
  /// is of interest.</param> <returns>The degree of similarity 0 to 1.0, where
  /// -1 represents a similarity lower than minSimilarity, otherwise, a number
  /// between 0 and 1.0 where 0 represents a lack of any noteable similarity,
  /// and 1 represents equivalent strings.</returns>
  double Similarity(xstring string1, xstring string2, double minSimilarity) {
    if (minSimilarity < 0 || minSimilarity > 1)
      throw "minSimilarity must be in range 0 to 1.0";
    if (string1.empty() || string2.empty())
    ...
    if (iMaxDistance < len2) {
      return Helpers::ToSimilarity(
          Distance(string1, string2, len1, len2, start, iMaxDistance,
                   this->baseChar1Costs, this->basePrevChar1Costs),
          string2.size());
    }
    return Helpers::ToSimilarity(
  ...
  /// <summary>Internal implementation of the core Damerau-Levenshtein, optimal
  /// string alignment algorithm.</summary>
  /// <remarks>https://github.com/softwx/SoftWx.Match</remarks>
  static int Distance(xstring string1, xstring string2, int len1, int len2,
                      int start, vector<int> char1Costs,
                      vector<int> prevChar1Costs) {
    int j;
    ...
    for (int i = 0; i < len1; ++i) {
      ...
      for (j = 0; j < len2; ++j) {
        ...
        prevChar1Costs[j] = currentCost =
            leftCharCost;              // cost of diagonal (substitution)
        leftCharCost = char1Costs[j];  // left now equals current cost (which
                                       // will be diagonal at next iteration)
        ...
        if (char1 != char2) {
          ...
          ++currentCost;
          if ((i != 0) && (j != 0) && (char1 == prevChar2) &&
              (prevChar1 == char2) && (thisTransCost + 1 < currentCost)) {
  ...
  /// <summary>Internal implementation of the core Damerau-Levenshtein, optimal
  /// string alignment algorithm that accepts a maxDistance.</summary>
  static int Distance(xstring string1, xstring string2, int len1, int len2,
                      int start, int maxDistance, vector<int> char1Costs,
                      vector<int> prevChar1Costs) {
    int i, j;
    ...
    for (i = 0; i < len1; ++i) {
      ...
      // cells (lower right diag is i - lenDiff) and the upper left diagonal +
      ...
      for (j = jStart; j < jEnd; ++j) {
        ...
        nextTransCost = prevChar1Costs[j];
        prevChar1Costs[j] = currentCost =
            leftCharCost;              // cost on diagonal (substitution)
        leftCharCost = char1Costs[j];  // left now equals current cost (which
                                       // will be diagonal at next iteration)
        xchar prevChar2 = char2;
        char2 = string2[start + j];
...
/// <summary>
/// Class providing optimized methods for computing Levenshtein comparisons
/// between two strings.
/// </summary>
/// <remarks>
/// Copyright ©2015-2018 SoftWx, Inc.
...
/// from Sten Hjelmqvist's "Fast, memory efficient" algorithm, described at
/// http://www.codeproject.com/Articles/13525/Fast-memory-efficient-Levenshtein-algorithm
/// The Levenshtein algorithm computes the edit distance metric between two
/// strings, i.e. the number of insertion, deletion, and substitution edits
/// required to transform one string to the other. This value will be >= 0,
/// where 0 indicates identical strings. Comparisons are case sensitive, so for
/// example, "Fred" and "fred" will have a distance of 1. The optimized
/// algorithm was described in detail in my post at
/// http://blog.softwx.net/2014/12/optimizing-levenshtein-algorithm-in-c.html
/// Also see http://en.wikipedia.org/wiki/Levenshtein_distance for general
/// information. The methods in this class are not threadsafe. Use the static
/// versions in the Distance class if that is required.</remarks>
class Levenshtein : public IDistance, ISimilarity {
 ...
 public:
  ...
  /// efficiency can be achieved.</remarks> <param
  /// name="expectedMaxstringLength">The expected maximum length of strings that
  /// will be passed to the Levenshtein methods.</param>
  ...
  /// <summary>Compute and return the Levenshtein edit distance between two
  /// strings.</summary> <remarks>https://github.com/softwx/SoftWx.Match This
  /// method is not threadsafe.</remarks> <param name="string1">One of the
  /// strings to compare.</param> <param name="string2">The other string to
  /// compare.</param> <returns>0 if the strings are equivalent, otherwise a
  /// positive number whose magnitude increases as difference between the
  /// strings increases.</returns>
  double Distance(xstring string1, xstring string2) {
    if (string1.empty()) return string2.size();
    if (string2.empty()) return string1.size();
    ...
    return Distance(string1, string2, len1, len2, start,
                    (this->baseChar1Costs =
                         (len2 <= static_cast<int>(this->baseChar1Costs.size()))
                             ? this->baseChar1Costs
                             : vector<int>(len2, 0)));
  }
  ...
  /// method is not threadsafe.</remarks> <param name="string1">One of the
  /// strings to compare.</param> <param name="string2">The other string to
  /// compare.</param> <param name="maxDistance">The maximum distance that is of
  /// interest.</param> <returns>-1 if the distance is greater than the
  /// maxDistance, 0 if the strings are equivalent, otherwise a positive number
  /// whose magnitude increases as difference between the strings
  /// increases.</returns>
  double Distance(xstring string1, xstring string2, double maxDistance) {
    ...
    if (iMaxDistance < len2) {
      return Distance(
          string1, string2, len1, len2, start, iMaxDistance,
          (this->baseChar1Costs =
               (len2 <= static_cast<int>(this->baseChar1Costs.size()))
                   ? this->baseChar1Costs
                   : vector<int>(len2, 0)));
  ...
  /// <param name="string2">The other string to compare.</param>
  /// <returns>The degree of similarity 0 to 1.0, where 0 represents a lack of
  /// any noteable similarity, and 1 represents equivalent strings.</returns>
  double Similarity(xstring string1, xstring string2) {
    if (string1.empty()) return (string2.empty()) ? 1 : 0;
    if (string2.empty()) return 0;
    ...
    return Helpers::ToSimilarity(
        Distance(string1, string2, len1, len2, start,
                 (this->baseChar1Costs =
                      (len2 <= static_cast<int>(this->baseChar1Costs.size()))
                          ? this->baseChar1Costs
                          : vector<int>(len2, 0))),
  ...
  /// <summary>Internal implementation of the core Levenshtein
  /// algorithm.</summary>
  /// <remarks>https://github.com/softwx/SoftWx.Match</remarks>
  static int Distance(xstring string1, xstring string2, int len1, int len2,
                      int start, vector<int>& char1Costs) {
    for (int j = 0; j < len2; j++) char1Costs[j] = j + 1;
        ...
        for (int j = 0; j < len2; ++j) {
          ...
          if (string2[j] != char1) {
            ...
            if (aboveCharCost < currentCharCost)
              currentCharCost = aboveCharCost;  // deletion
            if (leftCharCost < currentCharCost)
              currentCharCost = leftCharCost;  // insertion
            ++currentCharCost;
          }
  ...
  /// <summary>Internal implementation of the core Levenshtein algorithm that
  /// accepts a maxDistance.</summary>
  /// <remarks>https://github.com/softwx/SoftWx.Match</remarks>
  static int Distance(xstring string1, xstring string2, int len1, int len2,
                      int start, int maxDistance, vector<int>& char1Costs) {
    //            if (maxDistance >= len2) return Distance(string1, string2,
    //            len1, len2, start, char1Costs);
    int i, j;
    for (j = 0; j < maxDistance; j++) char1Costs[j] = j + 1;
    for (; j < len2;) char1Costs[j++] = maxDistance + 1;
    int lenDiff = len2 - len1;
        ...
        for (j = jStart; j < jEnd; ++j) {
          ...
          if (string2[j] != char1) {
            ...
            if (prevChar1Cost < currentCost)
              currentCost = prevChar1Cost;  // insertion
            ++currentCost;
          }
          char1Costs[j] = aboveCharCost = currentCost;
        }
...
class EditDistance {
 ...
 public:
  ...
  /// <summary>Compare a string to the base string to determine the edit
  /// distance, using the previously selected algorithm.</summary> <param
  /// name="string2">The string to compare.</param> <param
  /// name="maxDistance">The maximum distance allowed.</param> <returns>The edit
  /// distance (or -1 if maxDistance exceeded).</returns>
...
/// of time can be achieved, as well as a reduction in final memory
/// usage.</summary>
class SuggestionStage {
 ...
 public:
  ...
  /// <param name="initialCapacity">The expected number of words that will be
  /// added.</param>
  ...
  /// <summary>Gets the total count of all suggestions for all
  /// deletes.</summary>
  ...
  void Add(int deleteHash, xstring suggestion) {
    ...
    newEntry.first = -1;
    Entry entry =
        (deletesFinded == Deletes.end()) ? newEntry : deletesFinded->second;
  ...
  void CommitTo(
      std::shared_ptr<Dictionary<int, std::vector<xstring>>> permanentDeletes) {
...
class SuggestItem {
 public:
  ...
  int distance = 0;
  /// <summary>Frequency of suggestion in the dictionary (a measure of how
  /// common the word is).</summary>
  int64_t count = 0;
  ...
  int CompareTo(SuggestItem other) {
    // order by distance ascending, then by frequency count descending, and then
    // by alphabet descending
    int disCom = Helpers::CompareTo(this->distance, other.distance);
    if (disCom != 0) return disCom;
    int cntCom = Helpers::CompareTo(other.count, this->count);
    if (cntCom != 0) return cntCom;
    return this->term.compare(other.term);
  }

  bool Equals(const SuggestItem obj) {
    return this->term == obj.term && this->distance == obj.distance &&
           this->count == obj.count;
  }

  int GetHashCode() { return hash<xstring>{}(this->term); }

  xstring Tostring() {
    return XL("{") + term + XL(", ") + to_xstring(distance) + XL(", ") +
           to_xstring(count) + XL("}");
  }
