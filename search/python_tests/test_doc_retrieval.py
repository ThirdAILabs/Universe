...
def test_add_doc_find_centroids_is_fast():
    ...
    doc_index_precomputed_centroids = thirdai.search.DocRetrieval(
        centroids=centroids,
        hashes_per_table=8,
        num_tables=8,
        dense_input_dimension=data_dim,
    )
    doc_index_compute_centroids = thirdai.search.DocRetrieval(
        centroids=centroids,
        hashes_per_table=8,
        num_tables=8,
        dense_input_dimension=data_dim,
    ...
    for i, doc in enumerate(docs):
        doc_index_compute_centroids.add_doc(
            doc_id=str(i), doc_text="test", doc_embeddings=np.array(doc)
        )
