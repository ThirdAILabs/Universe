...
class Worker:
    ...
    def getDragonGradients(self, compression_density):
        ...
        self.network.get_compressed_gradients(
        ...
        for layer in range(len(self.layers) - 1):
            ...
            x = self.network.get_compressed_gradients(
                compression_scheme="dragon",
                layer_index=layer,
                compression_density=compression_density,
                sketch_biases=False,
            ...
            y = self.network.get_compressed_gradients(
                compression_scheme="dragon",
                layer_index=layer,
                compression_density=1,
                sketch_biases=True,
    ...
    def receiveDragonGradients(self):
        ...
        for layer in range(len(self.layers) - 1):
            ...
            w_compress = w_sparse_grads[0][layer]

            w_indices = np.ravel(
                np.hstack(
                    [node_weights[layer]["indices"] for node_weights in w_sparse_grads]
                )
            )
            ...
            w_values = np.ravel(
                np.hstack(
                    [
                        node_weights[layer]["values"] / num_workers
                        for node_weights in w_sparse_grads
                    ]
                )
            )
            b_compress = b_sparse_grads[0][layer]
            b_indices = np.ravel(
                np.hstack(
                    [node_biases[layer]["indices"] for node_biases in b_sparse_grads]
                )
            )
            b_values = np.ravel(
                np.hstack(
                    [
                        node_biases[layer]["values"] / num_workers
                        for node_biases in b_sparse_grads
                    ]
                )
            )
            w_compress["indices"] = w_indices
            w_compress["values"] = w_values
            b_compress["indices"] = b_indices
            b_compress["values"] = b_values

            self.network.set_compressed_gradients(
                layer_index=1, set_biases=False, compressed_vector=w_compress
            )
            self.network.set_compressed_gradients(
                layer_index=1, set_biases=False, compressed_vector=b_compress
            )
