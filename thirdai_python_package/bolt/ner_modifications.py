...
def modify_ner():
    ...
    def wrapped_train(
        ...
        train_data_source = NerDataSource(ner_model = self, file_path=filename)
        
        if validation_file:
            validation_data_source = NerDataSource(ner_model=self, file_path=validation_file)
        else:
    ...
    def wrapped_predict_batch(
        ...
        inference_source = NerDataSource(self, None)
