...
class SQLIterator:
    def __init__(
        ...
        # Since assigned chunk_ids are contiguous, each SQLIterator can search
...
class SQLChunkStore(ChunkStore):
    def __init__(
        ...
        **kwargs,
        ...
        Params:
            ...
            sql_uri: Optional[str] - SQL URI to use for creating chunk store tables
        """
        super().__init__()

        self.sql_uri = sql_uri
        self.engine = create_engine_with_fk(self.sql_uri)
    ...
    def insert(
        ...
        inserted_chunks_iterator = SQLIterator(
    ...
    def load(cls, path: str, encryption_key: Optional[str] = None, **kwargs):
        ...
        obj.sql_uri = obj._get_sql_uri(path)

        obj.engine = create_engine_with_fk(obj.sql_uri)
