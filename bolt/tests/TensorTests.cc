...
void checkTensorContents(const tensor::ActivationTensorPtr& tensor) {
  for (uint32_t i = 0; i < tensor->shape()[0]; i++) {
    for (uint32_t j = 0; j < tensor->shape()[1]; j++) {
      if (tensor->numNonzeros().value() < tensor->dim()) {
        uint32_t active_neuron =
            tensor->activeNeuronsPtr()[i * tensor->numNonzeros().value() + j];
        ASSERT_EQ(active_neuron, i);
      }

      float activation =
          tensor->activationsPtr()[i * tensor->numNonzeros().value() + j];
      ASSERT_EQ(activation, static_cast<float>(i));

      float gradient =
          tensor->gradientsPtr()[i * tensor->numNonzeros().value() + j];
...
TEST(TensorTests, ActivationTensor) {
  ...
  ASSERT_EQ(tensor->numNonzeros(), 4);
  ...
  ASSERT_EQ(tensor->numNonzeros(), 8);
  ...
  tensor->updateSparsity(/* new_sparse_nonzeros= */ 6);
  ...
  ASSERT_EQ(tensor->numNonzeros(), 6);
