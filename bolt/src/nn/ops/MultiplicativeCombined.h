...
class MultiplicativeCombinedEmbedding final
    : public Op,
      public std::enable_shared_from_this<MultiplicativeCombinedEmbedding> {
 public:
  static std::shared_ptr<MultiplicativeCombinedEmbedding> make(
  ...
  std::shared_ptr<MultiplicativeCombinedEmbedding> duplicateWithNewReduction(
      const std::string& reduction,
      std::optional<uint64_t> num_tokens_per_input);

 private:
  MultiplicativeCombinedEmbedding(uint64_t num_embedding_lookups,
  ...
  MultiplicativeCombinedEmbedding(
      std::unique_ptr<EmbeddingLayer>&& pos_kernel,
      std::unique_ptr<EmbeddingLayer>&& token_kernel, const std::string& name)
      : Op(name),
...
using MultiplicativeCombinedEmbeddingPtr =
