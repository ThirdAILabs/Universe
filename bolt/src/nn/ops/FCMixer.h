...
class FCMixer final : public Op, public std::enable_shared_from_this<FCMixer> {
 public:
  ...
  static auto cast(const ops::OpPtr& op) {
    return std::dynamic_pointer_cast<FCMixer>(op);
  }
...
using FCMixerPtr = std::shared_ptr<FCMixer>;
