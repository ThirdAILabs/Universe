#include "MultiplicativeCombined.h"
...
std::string nextMultiplicativeCombinedOpName() {
  static uint32_t constructed = 0;
  return "multiplicative_combined_" + std::to_string(++constructed);
}

MultiplicativeCombinedEmbedding::MultiplicativeCombinedEmbedding(
    ...
    : Op(nextMultiplicativeCombinedOpName()) {
...
std::shared_ptr<MultiplicativeCombinedEmbedding>
MultiplicativeCombinedEmbedding::make(
    uint64_t num_embedding_lookups, uint64_t lookup_size,
    uint64_t log_embedding_block_size, const std::string& reduction,
    std::optional<uint64_t> num_tokens_per_input, uint64_t update_chunk_size) {
  return std::shared_ptr<MultiplicativeCombinedEmbedding>(
      new MultiplicativeCombinedEmbedding(
          /* num_embedding_lookups= */ num_embedding_lookups,
          /* lookup_size= */ lookup_size,
          /* log_embedding_block_size= */ log_embedding_block_size,
          /* reduction= */ reduction,
...
void MultiplicativeCombinedEmbedding::forward(
  ...
  for (uint64_t token_idx = 0; token_idx < position_input.len; token_idx++) {
    position_input.active_neurons[token_idx] =
        (token_idx + 99371 * position_input.active_neurons[token_idx]);
  }
...
void MultiplicativeCombinedEmbedding::backpropagate(
...
void MultiplicativeCombinedEmbedding::updateParameters(float learning_rate,
...
uint32_t MultiplicativeCombinedEmbedding::dim() const {
  assert(_pos_kernel->getOutputDim() == _token_kernel->getOutputDim());
  return _pos_kernel->getOutputDim();
}

std::optional<uint32_t> MultiplicativeCombinedEmbedding::nonzeros(
...
void MultiplicativeCombinedEmbedding::disableSparseParameterUpdates() {
...
std::vector<std::vector<float>*> MultiplicativeCombinedEmbedding::parameters() {
  return {&_pos_kernel->getRawEmbeddingBlock(),
          &_token_kernel->getRawEmbeddingBlock()};
}

void MultiplicativeCombinedEmbedding::summary(
    std::ostream& summary, const autograd::ComputationList& inputs,
    const autograd::Computation* output) const {
  summary << "MultiplicativeCombinedEmbedding(" << name()
...
void MultiplicativeCombinedEmbedding::setSerializeOptimizer(
...
autograd::ComputationPtr MultiplicativeCombinedEmbedding::apply(
    autograd::ComputationPtr input) {
  return autograd::Computation::make(shared_from_this(), {std::move(input)});
}

template void MultiplicativeCombinedEmbedding::save(
    cereal::BinaryOutputArchive&) const;

template <class Archive>
void MultiplicativeCombinedEmbedding::save(Archive& archive) const {
  archive(cereal::base_class<Op>(this), _pos_kernel, _token_kernel);
}

template void MultiplicativeCombinedEmbedding::load(
    cereal::BinaryInputArchive&);

template <class Archive>
void MultiplicativeCombinedEmbedding::load(Archive& archive) {
  archive(cereal::base_class<Op>(this), _pos_kernel, _token_kernel);
...
std::shared_ptr<MultiplicativeCombinedEmbedding>
MultiplicativeCombinedEmbedding::duplicateWithNewReduction(
  ...
  std::string new_name =
      nextMultiplicativeCombinedOpName() + "_shared_" + name();
  return std::shared_ptr<MultiplicativeCombinedEmbedding>(
      new MultiplicativeCombinedEmbedding(
...
struct specialize<Archive,
                  thirdai::bolt::nn::ops::MultiplicativeCombinedEmbedding,
                  cereal::specialization::member_load_save> {};

}  // namespace cereal

CEREAL_REGISTER_TYPE(thirdai::bolt::nn::ops::MultiplicativeCombinedEmbedding)