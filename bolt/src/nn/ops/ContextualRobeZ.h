...
class ContextualRobeZ final
    : public Op,
      public std::enable_shared_from_this<ContextualRobeZ> {
 public:
  static std::shared_ptr<ContextualRobeZ> make(
  ...
  std::shared_ptr<ContextualRobeZ> duplicateWithNewReduction(
      const std::string& reduction,
      std::optional<uint64_t> num_tokens_per_input);

 private:
  ContextualRobeZ(uint64_t num_embedding_lookups, uint64_t lookup_size,
                  uint64_t log_embedding_block_size,
                  uint64_t num_tokens_per_input, uint64_t update_chunk_size);

  ContextualRobeZ(std::unique_ptr<EmbeddingLayer>&& kernel,
...
using ContextualRobeZPtr = std::shared_ptr<ContextualRobeZ>;
