...
static BoltGraph buildSingleHiddenLayerModel(bool sparse_hidden_layer) {
  ...
  auto hidden_layer =
      std::make_shared<FullyConnectedNode>(2000, 1, ActivationFunction::ReLU);
  hidden_layer->addPredecessor(input);

  auto output = std::make_shared<FullyConnectedNode>(
      /* expected_dim */ n_classes, ActivationFunction::Softmax);
  ...
  if (sparse_hidden_layer) {
    ...
    hidden_layer->setNodeSparsity(hidden_layer_sparsity);
  }
