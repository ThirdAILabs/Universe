...
uint32_t DlrmAttentionNode::outputDim() const {
...
std::shared_ptr<DlrmAttentionNode> DlrmAttentionNode::setPredecessors(
...
void DlrmAttentionNode::prepareForBatchProcessingImpl(uint32_t batch_size,
                                                      bool use_sparsity) {
  (void)use_sparsity;
...
void DlrmAttentionNode::forwardImpl(uint32_t vec_index,
...
void DlrmAttentionNode::backpropagateImpl(uint32_t vec_index) {
  BoltVector& fc_output = _fully_connected_node->getOutputVector(vec_index);
...
void DlrmAttentionNode::summarizeImpl(std::stringstream& summary,
                                      bool detailed) const {
  (void)detailed;
  summary << "(" << _fully_connected_node->name() << ", "
          << _embedding_node->name() << ") -> " << name()
          << "(DLRMDotProductFeatureInteraction): output_dim="
          << _compiled_state->_output_dim
          << " num_embedding_chunks=" << _compiled_state->_num_embedding_chunks
          << "\n";
}

Node::NodeState DlrmAttentionNode::getState() const {
...
float DlrmAttentionNode::fcOutputEmbeddingDotProduct(
...
template <bool FC_OUTPUT_DENSE>
void DlrmAttentionNode::fcOutputEmbeddingDotProductBackward(
...
float DlrmAttentionNode::embeddingDotProduct(const float* emb_1,
                                             const float* emb_2, uint32_t dim) {
  float total = 0.0;
...
void DlrmAttentionNode::embeddingDotProductBackward(
...
CEREAL_REGISTER_TYPE_SOURCE(thirdai::bolt::DlrmAttentionNode)
