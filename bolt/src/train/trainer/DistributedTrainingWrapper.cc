...
DistributedTrainingWrapper::validationAndSave() {
  if (_save_context) {
    std::string checkpoint_path = _save_context->prefix() + "_" +
                                  std::to_string(_steps_since_save) +
                                  ".checkpoint.bolt";
    std::string save_path = _save_context->prefix() + "_" +
                            std::to_string(_steps_since_save) + ".save.bolt";
