...
void DLRM::initializeNetworkState(uint32_t batch_size, bool force_dense) {
  ...
  _bottom_mlp_output = std::vector<BoltVector>(batch_size);
  _embedding_layer_output = std::vector<BoltVector>(batch_size);

  for (uint32_t b = 0; b < batch_size; b++) {
    const BoltVector& concat_vec = _concat_layer_state[b];
    _bottom_mlp_output[b] =
        BoltVector(nullptr, concat_vec.activations, concat_vec.gradients,
                   bottom_mlp_output_dim);

    _embedding_layer_output[b] =
        BoltVector(nullptr, concat_vec.activations + bottom_mlp_output_dim,
