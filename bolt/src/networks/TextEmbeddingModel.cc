...
void TextEmbeddingModel::forward(uint32_t batch_index,
  ...
  // _classifier.forward(batch_index, _token_embedding_output[batch_index],
  // output,
  //                     labels);

  _classifier.forward(batch_index, input[batch_index], output, labels);
}

void TextEmbeddingModel::backpropagate(uint32_t batch_index,
                                       dataset::MaskedSentenceBatch& input,
                                       BoltVector& output) {
  _classifier.backpropagate<true>(batch_index, input[batch_index], output);
