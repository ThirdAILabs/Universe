...
tensor::TensorPtr createTensorWithGrad(const tensor::TensorList& inputs,
  ...
  const BoltVector& vector = inputs.at(0)->index2d(0);
  ...
  if (!vector.isDense()) {
    ...
    for (uint32_t i = 0; i < vector.len; i++) {
      ...
      tensor->index2d(0).active_neurons[i] = vector.active_neurons[i];
    }
  } else {
...
RCAGradients explainNeuronHelper(model::ModelPtr& model,
  ...
  auto label =
      tensor::Tensor::sparse({/* batch_size= */ 1,
                             ...
                             /* nonzeros= */ 1);
  label->index2d(0).active_neurons[0] = neuron;
  label->index2d(0).activations[0] = 1.0;
  ...
  const BoltVector& input_vec = input->index2d(0);
...
RCAGradients explainPrediction(model::ModelPtr& model,
  ...
  uint32_t prediction = output->index2d(0).getHighestActivationId();
