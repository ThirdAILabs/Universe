...
bolt::ModelPtr NerUDTModel::getBoltModel(
    std::unordered_map<std::string, uint32_t>& tag_to_label, uint32_t fhr) {
  std::cout << "The total number of elements in the unordered_map is: "
            << tag_to_label.size() << std::endl;
  auto number_labels = getMaxLabelFromTagToLabel(tag_to_label);
  auto input = bolt::Input::make(fhr);
  ...
  auto output =
      bolt::FullyConnected::make(number_labels, hidden->dim(), 1, "softmax",
                                 /* sampling= */ nullptr, /* use_bias= */ true)
          ->apply(hidden);

  auto labels = bolt::Input::make(number_labels);
  auto loss = bolt::CategoricalCrossEntropy::make(output, labels);
  return bolt::Model::make({input}, {output}, {loss});
}

void NerUDTModel::initialize(uint32_t fhr, uint32_t number_labels) {
...
NerUDTModel::NerUDTModel(
    ...
    : _tokens_column(std::move(tokens_column)),
      ...
      _target_word_tokenizers(std::move(target_word_tokenizers)),
      _tag_to_label(std::move(tag_to_label)) {
  _bolt_model = getBoltModel(_tag_to_label, 100'000);
  initialize(100'000, getMaxLabelFromTagToLabel(_tag_to_label));
}
