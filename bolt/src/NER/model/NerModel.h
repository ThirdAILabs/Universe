...
class NerBackend {
 public:
    ...
    // initialize 
  virtual std::vector<std::vector<uint32_t>> getTags(std::vector<std::vector<std::string>> tokens) = 0;
...
class NerModel : public std::enable_shared_from_this<NerModel> {
    ...
    public:
        ...
        static std::shared_ptr<NerModel> fromArchive(
        ...
        static std::shared_ptr<NerModel> load_stream(
