...
//edited
inline SamplingConfig initialize_hash_function(hashing_function type_of_hash,uint64_t _dim, float _sparsity) {
  uint32_t num_tables_guess = 128;
  float safety_factor = 1;
  float range_pow_float = std::log2(num_tables_guess / (_sparsity * safety_factor));
  ...
  uint32_t num_tables = std::round(_sparsity * safety_factor * (1 << range_pow));
  uint32_t expected_num_elements_per_bucket = std::max<uint32_t>(_dim / (1 << range_pow), 1);
  uint32_t reservoir_size = 4 * expected_num_elements_per_bucket;
  std::string ht;
  if(type_of_hash == hashing_function::Densified_MinHash) { ht = "DensifiedMinHash"; }
  if(type_of_hash == hashing_function::DWTA) { ht = "DWTA"; }
  if(type_of_hash == hashing_function::SRP) { ht = "SRP"; }
  if(type_of_hash == hashing_function::Fast_SRP) { ht = "FastSRP";}
...
//edited
    ...
    if(sampling_config.hashes_per_table != 0 || sampling_config.num_tables != 0 || sampling_config.range_pow !=0 || sampling_config.reservoir_size !=0) {
      return;
    }
...
//edited
  ...
  FullyConnectedLayerConfig(uint64_t _dim, float _sparsity,
                            ActivationFunction _act_func)
        : FullyConnectedLayerConfig(_dim,_sparsity,_act_func,SamplingConfig()) {}
    //   : dim(_dim), sparsity(_sparsity), act_func(_act_func), sampling_config(initialize_hash_function(hashing_function::DWTAHashFunction,_dim,_sparsity)) {
    // checkSparsity(sparsity);
    //edited
