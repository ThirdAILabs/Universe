...
void EmbeddingLayer::forward(uint32_t batch_index,
  ...
  _hash_locs[batch_index].clear();
  _hash_locs[batch_index].reserve(tokens.size() * _num_embedding_lookups);

  for (uint32_t lookup_index = 0; lookup_index < _num_embedding_lookups;
       lookup_index++) {
    float* output_start = output.activations + getOutputOffset(lookup_index);

    for (uint32_t token : tokens) {
      uint64_t hash_loc = getHashLocForToken(token, lookup_index);
      _hash_locs[batch_index].push_back(hash_loc);
...
void EmbeddingLayer::backpropagate(uint32_t batch_index,
                                   const BoltVector& output) {
  uint32_t num_tokens = _hash_locs[batch_index].size() / _num_embedding_lookups;

  for (uint32_t lookup_index = 0; lookup_index < _num_embedding_lookups;
       lookup_index++) {
    const float* gradients = output.gradients + getOutputOffset(lookup_index);

    for (uint32_t token_index = 0; token_index < num_tokens; token_index++) {
      uint64_t hash_loc = _hash_locs[batch_index][getHashLocIndex(
