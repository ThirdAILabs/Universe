...
class HashTable {
 private:
  ...
  constexpr uint64_t CounterIdx(uint64_t table, uint64_t row) {
    return table * range + row;
  }
  ...
  constexpr uint64_t HashIdx(uint64_t i, uint64_t table) {
    return i * num_tables + table;
  }
