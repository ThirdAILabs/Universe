...
def gen_random_weights():
    np.random.seed(17)
    w1 = np.random.randn(3, 5).astype(np.float32)
...
def test_input_gradients():
    ...
    and when required labels passed as second best labels.
    """
    network = build_network()
    set_network_weights_and_biases(network)
    inputs = np.random.randn(3, 5).astype(np.float32)
    grad, offset = network.get_input_gradients(
        inputs, bolt.CategoricalCrossEntropyLoss()
    )
    grad1, offset1 = network.get_input_gradients(
        inputs, bolt.CategoricalCrossEntropyLoss(), np.array([0, 0, 1]).astype("uint32")
    )
    assert grad == grad1
