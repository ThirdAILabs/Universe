...
# checkpoints the network during training phase and after ending the training phase
# after ending the training, model should not save optimizer
# loading the model back from the two states and then training the loaded model
def test_end_resume_training():
    ...
    network.end_training()
    network.save_for_inference(save_loc)
    train_network(network, train_x, train_y, epochs=2)

    assert 2 * os.path.getsize(save_loc) < os.path.getsize(checkpoint_loc)

    inferrence_network = bolt.Network.load(save_loc)
    checkpointed_network = bolt.Network.load(checkpoint_loc)

    inferrence_acc, _ = inferrence_network.predict(
        test_x, test_y, metrics=["categorical_accuracy"], verbose=False
    )
    checkpointed_acc, _ = checkpointed_network.predict(
        test_x, test_y, metrics=["categorical_accuracy"], verbose=False
    )

    assert (
        inferrence_acc["categorical_accuracy"] == original_acc["categorical_accuracy"]
    )
    assert (
        checkpointed_acc["categorical_accuracy"] == original_acc["categorical_accuracy"]
    )

    train_network(inferrence_network, train_x, train_y, epochs=5)
    train_network(checkpointed_network, train_x, train_y, epochs=5)
...
# saves inferrence model and checkpointed model
# checks that both gives the same accuracy after loading
# trains the loaded models and checks that accuracy increases after training
def test_load_save_fc_network():
    ...
    train_network(inferrence_network, train_x, train_y, epochs=5)
    train_network(checkpointed_network, train_x, train_y, epochs=2)

    another_acc, _ = inferrence_network.predict(
        test_x, test_y, metrics=["categorical_accuracy"], verbose=False
    )
    another_acc_resume, _ = checkpointed_network.predict(
        test_x, test_y, metrics=["categorical_accuracy"], verbose=False
    )

    assert another_acc["categorical_accuracy"] >= inferrence_acc["categorical_accuracy"]
    assert (
        another_acc_resume["categorical_accuracy"]
        >= checkpointed_acc["categorical_accuracy"]
    )
...
def test_load_save_fc_network_big():
    network = build_sparse_hidden_layer_network(1000, 0.2)
    ...
    wrong_inferrence_network = bolt.Network.load(checkpoint_loc)
    wrong_checkpointed_network = bolt.Network.load(save_loc)
    ...
    wrong_inferrence_network.checkpoint(wrong_load_loc)
    wrong_checkpointed_network.checkpoint(wrong_resume_loc)
    ...
    wrong_inferrence_acc, _ = wrong_inferrence_network.predict(
        test_x, test_y, metrics=["categorical_accuracy"], verbose=False
    )
    wrong_checkpointed_acc, _ = wrong_checkpointed_network.predict(
        test_x, test_y, metrics=["categorical_accuracy"], verbose=False
    )
    assert (
        inferrence_acc["categorical_accuracy"] == original_acc["categorical_accuracy"]
    ...
    assert (
        wrong_checkpointed_acc["categorical_accuracy"]
        == original_acc["categorical_accuracy"]
    )
    assert (
        wrong_inferrence_acc["categorical_accuracy"]
        == original_acc["categorical_accuracy"]
    )
    ...
    train_network(inferrence_network, train_x, train_y, epochs=5)
    train_network(checkpointed_network, train_x, train_y, epochs=2)
    train_network(wrong_checkpointed_network, train_x, train_y, epochs=2)
    train_network(wrong_checkpointed_network, train_x, train_y, epochs=2)
