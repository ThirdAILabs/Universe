...
def build_train_and_predict(
    values_np,
    indices_np,
    offsets_np,
    labels_np,
    input_output_dim,
    optimize_sparse_sparse,
    output_sparsity=0.5,
    batch_size=256,
):
    data = dataset.from_numpy(
        (indices_np, values_np, offsets_np), batch_size=batch_size
    )
...
def test_sparse_sparse_above_threshold():
    ...
    res = build_train_and_predict(
        values_np,
        indices_np,
        offsets_np,
        labels_np,
        input_output_dim,
        optimize_sparse_sparse=True,
    )
