...
def predict_train_one_epoch_predict(network, test_data, test_labels, sparse_inference):
    ...
    of training, then does another forward pass. Returns a tuple of the prediction
    before the epoch of training and the prediction after.
    """
    prediction_before = network.predict(
        test_data=test_data, test_labels=None, sparse_inference=sparse_inference
    )[1:]
    train_network(
        network=network, train_data=test_data, train_labels=test_labels, epochs=1
    )
...
@pytest.mark.release
def test_switch_dense_to_sparse2(
    num_classes, input_dim, output_dim, sparsity, learning_rate
):
    train_data, train_labels = gen_training_data(n_classes=num_classes, n_samples=1000)
...
@pytest.mark.release
def test_decrease_and_increase_sparsity2():
    model = build_simple_graph_model(
        input_dim=20,
        output_dim=10,
        num_classes=10,
    ...
    model.set_sparsity(sparsity=0.5)
    assert model.get_sparsity() == 0.5

    model.set_sparsity(sparsity=0.35)
    assert model.get_sparsity() == 0.35


@pytest.mark.release
def test_switch_dense_to_sparse():
    """
    Tests that we can do both training and inference when switching from a dense
    final layer to a sparse final layer and back. We ensure that the last layer is
    truly sparse by exploiting the difference in behavior of a dense and sparse
    final layer with sparse inference enabled: with sparse inference enabled
    a dense final layer will return a single dense activations array from a
    prediction, while a sparse final layer will return both the activations and
    the indices of those activations.
    """
    dataset_dim = 100
    ...
    # expect the output to be sparse when the layer was set to a sparsity
    # < 1, and dense when it was set to sparsity = 1.
...
# This is not a release test because the sampling config isn't exposed in
# a release build
def test_decrease_and_increase_sparsity_sampling_config():
    """
    Tests that changing the sparsity of an already sparse layer changes the
    sampling config parameters. Due to the way we autotune, only the number
    of tables should change if we change the sparsity.
    """
