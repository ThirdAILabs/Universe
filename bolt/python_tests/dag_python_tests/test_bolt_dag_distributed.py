...
def test_distributed_training_with_bolt():
    ...
    for epoch in range(epochs):
        for batch_num in range(num_of_batches):
            ...
            avg_weight_gradients_fc_1 += model_a.get_layer("fc_1").weight_gradients.get()
            avg_weight_gradients_fc_2 += model_a.get_layer("fc_2").weight_gradients.get()

            avg_bias_gradients_fc_1 += model_a.get_layer("fc_1").bias_gradients.get()
            avg_bias_gradients_fc_2 += model_a.get_layer("fc_2").bias_gradients.get()
    ...
    #checks
    FC_1_WEIGHTS = (model_a.get_layer("fc_1").weights.get()==model_a.get_layer("fc_1").weights.get()).all()
    FC_2_WEIGHTS = (model_a.get_layer("fc_2").weights.get()==model_a.get_layer("fc_2").weights.get()).all()
    FC_1_BIASES = (model_a.get_layer("fc_1").biases.get()==model_a.get_layer("fc_1").biases.get()).all()
    FC_2_BIASES = (model_a.get_layer("fc_2").biases.get()==model_a.get_layer("fc_2").biases.get()).all()

    metrics_model_a = model_a.predict()
    metrics_model_b = model_b.predict()

    SAME_ACCURACY = (metrics_model_a["categorical_accuracy"]==metrics_model_b["categorical_accuracy"])
    ACCURACY_GREATER_THAN_THRESHOLD = (metrics_model_a["categorical_accuracy"]>0.8)
