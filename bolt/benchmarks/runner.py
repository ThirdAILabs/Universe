...
def train_mnist_sparse_output_layer(args):
    ...
    train_data = dataset.loadBoltDataset(args.train, 250, format="svm")
    test_data = dataset.loadBoltDataset(args.test, 250, format="svm")
    epoch_times = []
    epoch_accuracies = []
...
def train_amzn670(args):
    ...
    train_data = dataset.loadBoltDataset(args.train, 256, format="svm")
    test_data = dataset.loadBoltDataset(args.test, 256, format="svm")
    epoch_times = []
    epoch_accuracies = []
    for _ in range(args.epochs):
        times = network.Train(train_data, args.lr, 1,
