...
def test_get_set_values():
    model = build_simple_hidden_layer_model(input_dim=1000, hidden_dim=1000, output_dim=100)
    ...
    for i, values in enumerate(new_first_layer_weights):
        ...
        if values != 0:
            ...
            if( old_first_layer_weights[i] != new_first_layer_weights[i]):
                bruh+=1
                # print(old_first_layer_weights[i],new_first_layer_weights[i])
    print(f"total: {index} diff: {bruh}")
...
def test_compressed_dragon_vector_training():
    ...
    train_data, train_labels = gen_numpy_training_data(
        n_classes=N_CLASSES, n_samples=100000, convert_to_bolt_dataset=False
    )
    ...
    for epochs in range(1):
        for batch_num in tqdm(range(5),mininterval=5):
    ...
    print(f"total time: {round(end_time,4)}, compression time: {round(end_compression_time,4)}")
    ...
    print(f"accuracy: {acc[0]['categorical_accuracy']}")
    # assert acc[0]["categorical_accuracy"] >= ACCURACY_THRESHOLD

test_compressed_dragon_vector_training()