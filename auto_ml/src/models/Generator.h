...
class QueryCandidateGeneratorConfig {
 ...
 public:
  QueryCandidateGeneratorConfig(
      ...
      std::string target_column_name, char delimiter = ',',uint32_t batch_size = 10000,
  ...
  static std::shared_ptr<QueryCandidateGeneratorConfig> fromDefault(
      ...
      const std::string& target_column_name, const std::string& dataset_size, char delimiter) {
 ...
 private:
  ...
  void serialize(Archive& archive) {
    archive(_hash_function, _num_tables, _hashes_per_table, _batch_size, _range,
            _n_grams, _reservoir_size, _source_column_name, _target_column_name, _delimiter,
 ...
 private:
  explicit QueryCandidateGenerator(
    ...
    _inference_batch_processor =
        std::make_shared<dataset::GenericBatchProcessor>(
            ...
            /* has_header = */ false, /* delimiter = */ _query_generator_config->delimiter());
  }
  ...
  std::tuple<uint32_t, uint32_t> mapColumnNamesToIndices(
    ...
    auto column_number_map =
        std::make_shared<ColumnNumberMap>(*file_header, _query_generator_config->delimiter());
