...
class PretrainedBase : public std::enable_shared_from_this<PretrainedBase> {
 public:
  PretrainedBase(std::string input_column, std::vector<bolt::ModelPtr> models,
  ...
  bolt::ModelPtr model() const {
    if (_models.empty()) {
      throw std::invalid_argument(
          "Cannot access model attribute of pretrained base.");
    }
    return _models[0];
  }

  data::TextTokenizerPtr tokenizer() const { return _tokenizer; }

  size_t outputDim() const { return _combined_output_dim; }

  ar::ConstArchivePtr toArchive() const;

  static std::shared_ptr<PretrainedBase> fromArchive(
  ...
  static std::shared_ptr<PretrainedBase> load(const std::string& filename);

  static std::shared_ptr<PretrainedBase> load_stream(
...
using PretrainedBasePtr = std::shared_ptr<PretrainedBase>;
