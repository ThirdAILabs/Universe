...
class TrainTestParameters {
 public:
  TrainTestParameters(uint32_t rebuild_hash_tables_interval,
  ...
  uint32_t rebuildHashTablesInterval() const {
    return _rebuild_hash_tables_interval;
  }

  uint32_t reconstructHashFunctionsInterval() const {
 ...
 private:
  uint32_t _rebuild_hash_tables_interval;
  uint32_t _reconstruct_hash_functions_interval;
