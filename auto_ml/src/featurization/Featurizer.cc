...
data::TransformationPtr Featurizer::coldStartTransform(
  ...
  // TODO(Nicholas, David): Is this the correct ordering of checks for fast
  // approx/variable_len
  ...
  if (variable_length) {
    return std::make_shared<data::VariableLengthColdStart>(
        strong_column_names, weak_column_names, _text_dataset->labelColumn(),
        _text_dataset->textColumn(), *variable_length);
  }
