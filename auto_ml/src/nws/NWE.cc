...
float SRPKernel::on(const std::vector<float>& a, const std::vector<float>& b) const {
...
float ExponentialKernel::on(const std::vector<float>& a, const std::vector<float>& b) const {
  assert(a.size() == b.size());
  float r = 0;
  for (size_t i = 0; i < a.size(); i++) {
...
void NadarayaWatsonEstimator::train(std::vector<std::vector<float>> inputs, std::vector<float> outputs) {
