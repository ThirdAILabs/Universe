...
std::shared_ptr<udt::UDT> makePretrainedText(
    const ColumnDataTypes& input_data_types, uint32_t n_target_classes,
...
void defineAutomlInModule(py::module_& module) {
  ...
  py::class_<udt::UDT, std::shared_ptr<udt::UDT>>(module,
      ...
      .def(py::init(&makePretrainedText), py::arg("data_types"),
...
std::shared_ptr<udt::UDT> makePretrainedText(
    ...
    bool integer_target, const SpladeMachPtr& pretrained_model, char delimiter,
    const py::dict& options) {
  return std::make_shared<udt::UDT>(input_data_types, n_target_classes,
