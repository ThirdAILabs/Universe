...
export CIBW_BEFORE_ALL="sed -i s/mirror.centos.org/vault.centos.org/g /etc/yum.repos.d/*.repo && sed -i s/^#.*baseurl=http/baseurl=http/g /etc/yum.repos.d/*.repo && sed -i s/^mirrorlist=http/#mirrorlist=http/g /etc/yum.repos.d/*.repo && yum update -y && yum clean all && yum install openssl11 openssl11-devel -y && openssl version -a && which openssl && which openssl11 && openssl11 version -a && mv /usr/bin/openssl /usr/bin/openssl_backup && ln -s /usr/bin/openssl11 /usr/bin/openssl && openssl version -a && yum install -y libxslt-devel libxml2-devel"
