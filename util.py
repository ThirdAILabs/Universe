...
def data_generator_dense(files, batch_size, feat_dim, n_classes):
    while 1:
        lines = []
        for file in files:
            with open(file,'r',encoding='utf-8') as f:
                ...
                while True:
                    ...
                    if len(lines)!=batch_size:
                        break
                    x_batch = np.zeros([batch_size,feat_dim], dtype=np.float32)
                    ...
                    for line in lines:
                        ...
                        y_idxs = itms[0].split(',')
                        for i in range(len(y_idxs)):
                            y_batch[count,int(y_idxs[i])] = 1.0/len(y_idxs)
                            # y_batch[count,y_idxs[i]] = 1.0
                        ##
                        for i in range(1,len(itms)):
                            x_batch[count, int(itms[i].split(':')[0])] = np.float32(itms[i].split(':')[1])
                        ##
                        count += 1
                    lines = []
