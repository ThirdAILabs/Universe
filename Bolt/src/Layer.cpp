...
Layer::Layer(uint64_t _dim, uint64_t _prev_dim, float _sparsity, ActivationFunc _act_func,
  ...
  if (sparsity < 1.0) {
    ...
    hash_table = new HashTable<uint32_t, uint32_t>(
        sampling_config.L, sampling_config.reservoir_size, sampling_config.range_pow);
...
void Layer::ForwardPass(uint32_t batch_indx, const uint32_t* indices, const float* values,
...
template void Layer::BackPropagate<true>(uint32_t, const uint32_t*, const float*, float*, uint32_t);

template void Layer::BackPropagate<false>(uint32_t, const uint32_t*, const float*, float*,
                                          uint32_t);

template <bool FIRST_LAYER>
void Layer::BackPropagate(uint32_t batch_indx, const uint32_t* indices, const float* values,
  ...
  for (uint64_t n = 0; n < active_lens[batch_indx]; n++) {
    for (uint64_t i = 0; i < len; i++) {
      ...
      if (values[i] > 0) {
        ...
        if (!FIRST_LAYER) {
          prev_errors[i] += errors[batch_indx][n] *
...
void Layer::ComputeErrors(uint32_t batch_indx, const uint32_t* labels, uint32_t label_len) {
...
void Layer::SelectActiveNeurons(uint32_t batch_indx, const uint32_t* indices, const float* values,
                                uint32_t len, uint32_t* labels, uint32_t label_len) {
  if (sparsity == 1.0) {
...
void Layer::UpdateParameters(float lr, uint32_t iter, float B1, float B2, float eps) {
...
#pragma omp parallel for default(none) shared(lr, B1, B1_, B2, B2_, eps)
  for (uint64_t n = 0; n < dim; n++) {
    ...
    for (uint64_t i = 0; i < prev_dim; i++) {
      ...
      weights[indx] += lr * (w_momentum[indx] / B1_) / (sqrt(w_velocity[indx] / B2_) + eps);
...
void Layer::BuildHashTables() {
  ...
  // TODO: hashes could be array with size max(batch size, dim) that is allocated once
  uint32_t* hashes = new uint32_t[num_tables * dim];

#pragma omp parallel for default(none) shared(num_tables, hashes)
  for (uint64_t n = 0; n < dim; n++) {
    hasher->HashVector(weights + n * prev_dim, prev_dim, hashes + n * num_tables);
  }
