...
Network::Network(std::vector<LayerConfig> configs, uint64_t input_dim)
  ...
  for (uint32_t i = 0; i < num_layers; i++) {
    ...
    layers[i] = new Layer(configs[i].dim, prev_dim, configs[i].sparsity, configs[i].act_func,
                          configs[i].sampling_config);
  }

  auto end = std::chrono::high_resolution_clock::now();

  std::cout << "\033[1;36mInitialized Network in "
...
#pragma omp parallel for default(none) shared(batch, batch_size)
  for (uint32_t b = 0; b < batch_size; b++) {
    ...
    for (uint32_t l = 1; l < num_layers - 1; l++) {
      layers[l]->ForwardPass(b, layers[l - 1]->GetIndices(b), layers[l - 1]->GetValues(b),
                             layers[l - 1]->GetLen(b));
    }
    layers[num_layers - 1]->ForwardPass(
        b, layers[num_layers - 2]->GetIndices(b), layers[num_layers - 2]->GetValues(b),
        layers[num_layers - 2]->GetLen(b), batch.labels[b], batch.label_lens[b]);

    /**
     * 2. Compute Errors
     */
    layers[num_layers - 1]->ComputeErrors(b, batch.labels[b], batch.label_lens[b]);
    ...
    for (uint32_t l = num_layers - 1; l > 0; l--) {
      layers[l]->BackPropagate<false>(b, layers[l - 1]->GetIndices(b), layers[l - 1]->GetValues(b),
                                      layers[l - 1]->GetErrors(b), layers[l - 1]->GetLen(b));
    }

    layers[0]->BackPropagate<true>(b, batch.indices[b], batch.values[b], nullptr, batch.lens[b]);
  }
...
#pragma omp parallel for default(none) shared(batch, batch_size, correct)
  for (uint32_t b = 0; b < batch_size; b++) {
    ...
    if (std::find(batch.labels[b], batch.labels[b] + batch.label_lens[b], pred) !=
...
void Network::Train(uint32_t batch_size, std::string train_data, std::string test_data,
                    float learning_rate, uint32_t epochs, uint32_t rehash_in, uint32_t rebuild_in,
  ...
  uint64_t intermediate_test_batches = std::min<uint64_t>(test.NumBatches(), max_test_batches);
  uint64_t intermediate_test_vecs =
      std::min<uint64_t>(test.NumVecs(), intermediate_test_batches * batch_size);
  ...
  if (intermediate_test_batches > 0) {
    ...
    std::cout << "\033[1;32mBefore training accuracy: " << ((double)correct / intermediate_test_vecs)
              << " (" << correct << "/" << intermediate_test_vecs << ")\033[0m" << std::endl;
  }
  ...
  for (uint32_t epoch = 0; epoch < epochs; epoch++) {
    ...
    std::cout << "\033[1;36mProcessed " << intermediate_test_batches << " test batches in "
              << std::chrono::duration_cast<std::chrono::seconds>(test_end - test_start).count()
              << " seconds\033[0m" << std::endl;

    std::cout << "\033[1;32mAccuracy: " << ((double)correct / intermediate_test_vecs) << " (" << correct
  ...
  std::cout << "\033[1;32mAfter training accuracy: " << ((double)final_correct / test.NumVecs())
            << " (" << final_correct << "/" << test.NumVecs() << ")\033[0m" << std::endl;
}

#ifdef PYTHON_BINDINGS

py::array_t<uint32_t> Network::PredictClasses(const Batch& batch, uint64_t batch_size) {
  if (batch_size != batch_size_hint) {
    for (uint32_t layer = 0; layer < num_layers; layer++) {
      layers[layer]->SetBatchSize(batch_size);
    }
...
#pragma omp parallel for default(none) shared(batch, batch_size, predictions)
  ...
  py::capsule free_when_done(predictions, [](void* ptr) { delete (uint32_t*)ptr; });

  return py::array_t<uint32_t>({batch_size}, {sizeof(uint32_t)}, predictions, free_when_done);
}

py::array_t<float> Network::GetWeightMatrix(uint32_t layer_index) {
  ...
  size_t prev_dim = (layer_index > 0) ? configs[layer_index - 1].dim : input_dim;

  return py::array_t<float>({dim, prev_dim}, {prev_dim * sizeof(float), sizeof(float)}, mem,
                            free_when_done);
}

py::array_t<float> Network::GetBiasVector(uint32_t layer_index) {
  if (layer_index >= num_layers) {
    return py::none();
  }
