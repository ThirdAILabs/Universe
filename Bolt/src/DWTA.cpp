...
DWTAHashFunction::DWTAHashFunction(uint32_t input_dim, uint32_t _K, uint32_t _L, uint32_t range_pow)
    : K(_K),
      L(_L),
      num_hashes(K * L),
      range(1 << range_pow),
...
void DWTAHashFunction::HashVector(const float* data, uint32_t len, uint32_t* final_hashes) {
...
uint32_t* DWTAHashFunction::HashSparseVector(const uint32_t* indices, const float* values,
                                             uint32_t len) {
  uint32_t* final_hashes = new uint32_t[L];
...
void DWTAHashFunction::HashSparseVector(const uint32_t* indices, const float* values, uint32_t len,
