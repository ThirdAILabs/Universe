...
class DWTAHashFunction {
 private:
  uint32_t K, L, num_hashes, log_num_hashes, range, dim, binsize, log_binsize, permute;
  uint32_t* bin_map;
  uint32_t* positions;
  ...
  constexpr uint32_t RandDoubleHash(int binid, int count) {
    ...
    uint32_t result = (rand_double_hash_seed * tohash << 3) >> (32 - this->log_num_hashes);
    return result;
  }
 ...
 public:
  DWTAHashFunction(uint32_t input_dim, uint32_t _K, uint32_t _L, uint32_t range_pow);
