...
ColdStartTextAugmentation::ColdStartTextAugmentation(
  ...
  if (weak_sample_reps <= 0){
    throw std::invalid_argument("Invalid parameter: weak_sample_reps "
  ...
  if (_weak_min_len && _weak_max_len &&
    ...
    throw std::invalid_argument("Invalid length parameter: weak_min_len "
  ...
  if (_strong_sample_num_words && _strong_max_len &&
    ...
    throw std::invalid_argument("Invalid parameter: strong_sample_num_words "
...
ColdStartTextAugmentation::Phrase ColdStartTextAugmentation::getStrongPhrase(std::string& s){
...
ColdStartTextAugmentation::Phrase ColdStartTextAugmentation::splitByWhitespace(std::string& s){
...
ColdStartTextAugmentation::PhraseCollection ColdStartTextAugmentation::getWeakPhrases(std::string& s){
  ...
  while (phrase_end != s.end()){
    ...
    while (phrase_stream >> word){
      ...
      if (_weak_max_len && current_natural_phrase.size() >= _weak_max_len.value()){
...
ColdStartTextAugmentation::PhraseCollection ColdStartTextAugmentation::sampleFromPhrases(
    ColdStartTextAugmentation::PhraseCollection &phrases, uint32_t num_to_sample, uint32_t num_reps){
  // Only iterate over the original phrases, as we append new ones to the end.
  if (num_reps == 0){
    throw std::invalid_argument("Invalid number of sampling repetitions: "
                                "must be greater than 0.");
  }
...
void ColdStartTextAugmentation::mergeStrongWithWeak(
    ColdStartTextAugmentation::PhraseCollection &weak_phrases, Phrase &strong_phrase){
