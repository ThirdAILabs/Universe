...
std::vector<Batch> createBatches(const std::vector<DenseVector>& input_vectors,
  ...
  while (current_vector_index < input_vectors.size()) {
    ...
    for (uint32_t batch_vec = 0; batch_vec < next_batch_size; batch_vec++) {
      next_batch._values[batch_vec] = input_vectors.at(batch_vec + current_vector_index).values.data();
    }
...
/**
 * This test generates 100 test vectors and 100 other vectors that have a
 * 0.9 similarity with those vectors. The test then generates 99,900 other
 * random vectors. The test then ensures that Flash built with 100 tables and
 * all of the generated vectors has 100% top-1 accuracy with the original 100
