...
print(
    f"Loading and indexing {num_vectors} vectors ({num_bytes//1000000000}GB) took {end - start}s",
    flush=True,
)
...
final_model = tf.keras.Model(
    inputs=raw_model.input, outputs=raw_model.get_layer("fc2").output
)
...
def do_ann():
    if request.method == "POST":
        ...
        try:
            ...
            image_batch_processed = tf.keras.applications.vgg16.preprocess_input(
                image_batch.copy()
            )
