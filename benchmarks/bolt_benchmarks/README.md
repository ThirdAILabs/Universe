...
The config file is a toml file that contains information about the dataset, model architecture, and other training parameters. These files are located in `benchmarks/bolt_benchmarks/configs` and are saved with the .txt extension so that they can be uploaded as artifacts to mlflow. These config files can specify any DAG Model. An example script for a simple sequential model is `configs/mnist_sh.txt` and an example for a two-tower model is `configs/criteo_dlrm.txt`. 
