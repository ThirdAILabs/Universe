...
import mlflow
from utils import start_mlflow_helper, log_machine_info
...
def run_benchmark(config):
    ...
    model.train(config.train_file, epochs=config.num_epochs, learning_rate=config.learning_rate, metrics=["categorical_accuracy"], callbacks=[])
...
def main():
    ...
    start_mlflow_helper(experiment_name=args.config_name, run_name=args.run_name, dataset=args.config_name, model_name='udt')
    log_machine_info()
