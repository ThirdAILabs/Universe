...
class TabularMetadata {
 public:
  ...
  uint32_t getClassId(const std::vector<std::string_view>& input_row) {
    return _class_to_class_id[input_row[label_col_index]];
  }
...
class TabularMetadataProcessor : public ComputeBatchProcessor {
 public:
  ...
  void processHeader(const std::string& header) final {
    ...
    if (column_names.size() != _column_dtypes.size()) {
      throw std::invalid_argument(
          "Mismatching number of columns in csv. Based on the datatypes "
          "specified, expected " +
          std::to_string(_column_dtypes.size()) + " columns but received " +
          std::to_string(column_names.size()) + " columns.");
    }
    ...
    for (uint32_t col = 0; col < _column_dtypes.size(); col++) {
      switch (_column_dtypes[col]) {
        case TabularDataType::Numeric: {
          ...
          if (_col_to_max_val[val] < val) {
            _col_to_max_val[val] = val;
          }
          if (_col_to_min_val[val] > val) {
            _col_to_min_val[val] = val;
          }
          break;
        }
        ...
        case TabularDataType::Label: {
          if (!_class_to_class_id.count(values[col])) {
            uint32_t label = _class_id_to_class.size();
            _class_to_class_id[values[col]] = label;
            _class_id_to_class.push_back(std::string(values[col]));
          }
          break;
        }
      }
