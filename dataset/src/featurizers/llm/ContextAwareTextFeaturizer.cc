...
ContextAwareTextFeaturizer::getContext(const json& line_content) {
  ...
  std::vector<uint32_t> target_tokens = token_encoding::tokenIds(
      thirdai::dataset::TextGenerationFeaturizer::getStringField(line_content,
                                                                 "target"));

  if (line_content.contains("context")) {
    std::vector<uint32_t> context_tokens = token_encoding::tokenIds(
        thirdai::dataset::TextGenerationFeaturizer::getStringField(line_content,
                                                                   "context"));
...
std::vector<uint32_t> ContextAwareTextFeaturizer::getPrompt(
  ...
  if (line_content.contains("prompt")) {
    return token_encoding::tokenIds(
        thirdai::dataset::TextGenerationFeaturizer::getStringField(line_content,
                                                                   "prompt"));
  }
  return {};
