...
class TabularPairGram : public Block {
 public:
  ...
  std::string giveMessage(float gradient_ratio_value,std::unordered_map<uint32_t, std::string> col_num_col_name_map,float row_ratio_sum) const final {
    std::string message = "The timestamp column  "+col_num_col_name_map.at(_metadata->getLabelCol())+" is  "+std::to_string(((gradient_ratio_value)/(row_ratio_sum))*100)+"% responsible.";
    return message;
  }
