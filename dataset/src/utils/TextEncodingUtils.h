...
class TextEncodingUtils {
 public:
   ...
   */
  static std::pair<std::vector<uint32_t>,std::optional<std::unordered_map<uint32_t,std::string>>> computeRawUnigramsWithRange(
      const std::string_view sentence, uint32_t output_range,bool want_map = false) {
    std::vector<uint32_t> unigrams;
  ...
  inline static void forEachWordHash(const std::string_view sentence,
    ...
    static_assert(
        std::is_convertible<WORD_PROCESSOR_T,
                            std::function<void(uint32_t, std::string)>>::value);
    ...
    for (uint32_t i = 0; i < sentence.size(); i++) {
      ...
      if (!prev_is_space && std::isspace(sentence[i])) {
        ...
        std::string r_word(sentence.data() + start_of_word_offset,
                           sentence.data() + start_of_word_offset + len);
