...
struct CountMinSketch {
  CountMinSketch(size_t n_rows, uint32_t range_pow, std::vector<float>& sketch,
                 std::vector<uint32_t>& hash_seeds)
      : _n_rows(n_rows),
        _mask(std::numeric_limits<uint32_t>::max() << (32 - range_pow) >>
              (32 - range_pow)),
        _range(1 << range_pow),
  ...
  void clear() {
    std::fill(_sketch.begin() + _sketch_offset,
              _sketch.begin() + _sketch_offset + (_range * _n_rows), 0.0);
  }
 ...
 private:
  ...
  size_t getIthStart(size_t i) const { return _sketch_offset + i * _range; }
...
struct TimeBoundCountMinSketch {
  ...
  void setVerbose(bool verbosity) const {
    primary->setVerbose(verbosity);
    secondary->setVerbose(verbosity);
  }

  void swapSketches() {
    auto new_primary = std::move(secondary);
    secondary = std::move(primary);
    primary = std::move(new_primary);
  }
 ...
 public:
  ...
  float query(uint32_t id, uint32_t start_timestamp, uint32_t range) const {
    ...
    while (day != end_day) {
      ...
      uint32_t next_interval_starts_on_day =
          !(day & (((1 << next_cms_idx) - 1)));
      while (day + next_interval_size < end_day && next_cms_idx < _n_sketches &&
             next_interval_starts_on_day) {
        next_cms_idx++;
      ...
      if (_verbose) {
        std::cout << "day " << day - start_day << " interval size "
                  << (1 << cms_idx) << " days" << std::endl;
      }
