...
/**
 * Treats the categorical identifiers as a character-delimited list  
 * of numeric IDs from a contiguous range. 
 * i.e. given ID = id1,id2,id3, indices of nonzeros = id1 % dim,
 * id2 % dim, id3 % dim.
 *
 ...
 */
class MultiNumericId : public CategoricalEncoding {
 public:
  /**
   * Constructor. Accepts the desired dimension of the encoding.
   */
  explicit MultiNumericId(char delimiter, uint32_t dim) : _delimiter(delimiter), _dim(dim) {}
