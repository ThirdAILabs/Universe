...
class AtomicCircularBuffers {
 public:
  explicit AtomicCircularBuffers(size_t n_buffers, size_t size) : _n_buffers(n_buffers), _size(size), _insert_idxs(n_buffers), _buffer(n_buffers * size) {}
 ...
 public:
  ...
  void index(uint32_t id, uint32_t timestamp, std::string_view categorical_attr) {
    if (id >= _index.n_buffers()) {
      std::cout << "WARNING: Tried to index with id " << id << " into a CategoricalHistoryIndex for " << _index.n_buffers() << " classes. Treated as a no-op." << std::endl;
      return;
    }
  ...
  size_t startIdx(uint32_t id) const {
    if (id >= _index.n_buffers()) {
      std::cout << "WARNING: Tried to get buffer for id " << id << " from a CategoricalHistoryIndex for " << _index.n_buffers() << " classes. Treated as a no-op." << std::endl;
      return 0;
    }
