...
//     size_t buffer_shift_days = std::max(0, static_cast<int>(days_after_start) - static_cast<int>(_buffer.size() - 1));
//     if (buffer_shift_days > 0) {
//       for (size_t i = 0; i < std::min(buffer_shift_days, _buffer.size()); i++) {
...
//     auto idx = ((timestamp - _start_timestamp) / SECONDS_IN_DAY + _start_idx) % _buffer.size();

//     _buffer[idx] += inc;
//   }

//   float query(uint32_t start_timestamp, uint32_t range) const {
//     auto start_offset = (start_timestamp - _start_timestamp) % _buffer.size();
//     uint32_t idx = (_start_idx + start_offset) % _buffer.size();
//     float sum = 0;
...
void forEachInSynthesizedData(size_t n_users, size_t n_days, std::function<void(uint32_t user, uint32_t timestamp, float count)> callback) {
...
int main(int argc, const char** argv) {
  ...
  forEachInSynthesizedData(
    ...
    if (i % 100000 == 0) {
      std::cout << "i " << i << " , " << approx.query(user_id, timestamp - 30 * SECONDS_IN_DAY, 30) << std::endl;
    }
    i++;
  });
