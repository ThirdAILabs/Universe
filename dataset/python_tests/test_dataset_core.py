...
def test_loader_shuffle():
    ...
    assert dataset_internal.dense_bolt_dataset_is_permutation_of_dense_matrix(
        input_ds_shuf_unseed_1, float_2d_mat
    )
    assert dataset_internal.dense_bolt_dataset_is_permutation_of_dense_matrix(
        input_ds_shuf_unseed_2, float_2d_mat
    )
    assert dataset_internal.dense_bolt_dataset_is_permutation_of_dense_matrix(
        input_ds_shuf_seed_1, float_2d_mat
    )
    assert dataset_internal.dense_bolt_dataset_is_permutation_of_dense_matrix(
        input_ds_shuf_seed_2, float_2d_mat
    )

    # For each of the processed datasets above, input must be the same as target;
    # they must follow the same permutation
    assert dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_unseed_1, target_ds_shuf_unseed_1
    )
    assert dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_unseed_2, target_ds_shuf_unseed_2
    )
    assert dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_seed_1, target_ds_shuf_seed_1
    )
    assert dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_seed_2, target_ds_shuf_seed_2
    )

    # To check that the shuffled datasets are indeed shuffled, make sure that they are not equal to the
    # unshuffled dataset.
    assert not dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_unseed_1, input_ds_unshuf
    )
    assert not dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_unseed_2, input_ds_unshuf
    )
    assert not dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_seed_1, input_ds_unshuf
    )
    assert not dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_seed_2, input_ds_unshuf
    )
    ...
    assert not dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_unseed_1, input_ds_shuf_unseed_2
    )
    assert dataset_internal.dense_bolt_datasets_are_equal(
        input_ds_shuf_seed_1, input_ds_shuf_seed_2
    )
