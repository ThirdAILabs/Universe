...
std::vector<std::vector<float>> makeFloatMatrix(uint32_t n_rows, uint32_t n_cols) {
  assert(n_cols >= 1);
...
std::vector<std::string> makeStringMatrix(std::vector<std::vector<float>>& float_matrix) {
  assert(n_cols >= 1);
...
std::vector<std::shared_ptr<Block>> makeMockBlocks(std::vector<bool> dense_configs) {
  std::vector<std::shared_ptr<Block>> blocks;
  for (uint32_t i = 0; i < dense_configs.size(); i++) {
    blocks.push_back(std::make_shared<MockBlock>(/* column = */ i, /* dense = */ dense_configs[i]));
  }
  return blocks;
}

void checkMatrixAndProcessedBatchEquality(std::vector<std::vector<float>>& matrix, std::optional<BoltDataLabelPair<bolt::BoltBatch>>& processed, bool expect_input_dense, bool expect_label_dense) {
...
TEST(GenericBatchProcessorTest, DenseInputDenseLabel) {
  ...
  checkMatrixAndProcessedBatchEquality(float_batch, processed_batch, /* expect_input_dense = */ true, /* expect_label_dense = */ true);
}
...
TEST(GenericBatchProcessorTest, SparseInputDenseLabel) {
  ...
  GenericBatchProcessor processor(makeMockBlocks({false, false, false}),
                                     makeMockBlocks({true, true, true}));
  auto processed_batch = processor.createBatch(string_batch);
  checkMatrixAndProcessedBatchEquality(float_batch, processed_batch, /* expect_input_dense = */ false, /* expect_label_dense = */ true);
}
...
TEST(GenericBatchProcessorTest, SparseInputSparseLabel) {
  ...
  auto processed_batch = processor.createBatch(string_batch);
  checkMatrixAndProcessedBatchEquality(float_batch, processed_batch, /* expect_input_dense = */ false, /* expect_label_dense = */ false);
}
...
TEST(GenericBatchProcessorTest, DenseInputSparseLabel) {
  ...
  GenericBatchProcessor processor(makeMockBlocks({true, true, true}),
                                     makeMockBlocks({false, false, false}));
  auto processed_batch = processor.createBatch(string_batch);
  checkMatrixAndProcessedBatchEquality(float_batch, processed_batch, /* expect_input_dense = */ true, /* expect_label_dense = */ false);
}
